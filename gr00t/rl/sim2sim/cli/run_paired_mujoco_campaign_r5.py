#!/usr/bin/env python3
"""Run the r5 full-resolved-action-warp paired MuJoCo campaign on the READY Student."""
...
R5_CONTROL_MODE = "MUJOCO_NATIVE_POSITION_TRUE_100_45_R5_FULL_ACTION_WARP"
...
def _norm_summary(norms: list[float]) -> dict[str, float]:
    material = np.asarray(norms, dtype=np.float64)
    return {
        "min": float(np.min(material)),
        "p25": float(np.percentile(material, 25)),
        "p50": float(np.percentile(material, 50)),
        "p75": float(np.percentile(material, 75)),
...
def _run_case(
    *,
    ...
    if actuator_map.door_hinge_actuator_id != 0 or actuator_map.handle_actuator_id != 1:
        raise ValueError("r5 campaign requires the two door actuators first")
    ...
    base_axis_clip_counts = np.zeros(5, dtype=np.int64)
    base_axis_at_cap_counts = np.zeros(5, dtype=np.int64)
    final_20d_clip_steps = 0
    physical_base_norms: list[float] = []
    scaled_unclipped_base_norms: list[float] = []
    ...
    with (
        ...
        for policy_step in range(horizon_policy_steps):
            ...
            effective_arm_actions.append(
                warped.stage_action.effective_high_level_action[0, 5:11].numpy().copy()
            )
            base_axis_clip_counts += (
                warped.base.axis_clipped.squeeze(0).to(torch.int64).numpy()
            )
            base_axis_at_cap_counts += (
                warped.base.axis_at_cap.squeeze(0).to(torch.int64).numpy()
            )
            final_20d_clip_steps += int(warped.simulator_action_clipped.any())
            physical_base_norms.append(
                float(torch.linalg.vector_norm(warped.base.physical[:, :3]))
            )
            scaled_unclipped_base_norms.append(
                float(torch.linalg.vector_norm(warped.base.scaled_unclipped[:, :3]))
            )
            policy_steps += 1
            ...
            for substep in range(4):
                ...
                row = {
                    ...
                    "applied_action": warped.logical_action.squeeze(0).tolist(),
                ...
                json.dumps(
                    {
                        ...
                        "raw_arm_delta_echo": warped.stage_action.raw_arm_delta_echo.squeeze(0).tolist(),
                        "effective_arm_delta": warped.stage_action.effective_high_level_action[0, 5:11].tolist(),
                    },
                    separators=(",", ":"),
                    allow_nan=False,
                )
    ...
    receipt = {
        "schema": "doordog.sim2sim.paired_mujoco_episode_receipt.r5.v1",
        ...
        "action_warp_diagnostics": {
            "mode": "R5_FULL_RESOLVED_ACTION_WARP",
            "thresholds_source": "RESOLVED_CONFIG_SNAPSHOT",
            "base_clip_low": list(warp_contract.base_low),
            "base_clip_high": list(warp_contract.base_high),
            "base_command_clip_steps_by_axis": base_axis_clip_counts.tolist(),
            "base_command_at_cap_steps_by_axis": base_axis_at_cap_counts.tolist(),
            "final_20d_action_clip_steps": final_20d_clip_steps,
            "physical_base_command_norm_first3": _norm_summary(physical_base_norms),
            "scaled_unclipped_base_command_norm_first3": _norm_summary(
                scaled_unclipped_base_norms
            ),
        },
        "stage_contract": {
...
def main() -> None:
    ...
    parser.add_argument("--action-warp-receipt", required=True, type=Path)
    ...
    if (
        ...
        or standing_gate["control_mode"] != R5_CONTROL_MODE
    ):
        raise ValueError("r5 campaign requires the full-action-warp standing-vitals PASS")
    action_warp_receipt_path = args.action_warp_receipt.resolve(strict=True)
    action_warp_receipt = json.loads(action_warp_receipt_path.read_text(encoding="utf-8"))
    if (
        action_warp_receipt["result"] != "PASS"
        or action_warp_receipt["coverage"] != "NONE_MISSING"
    ):
        raise ValueError("r5 campaign requires the complete action-warp PASS")
    ...
    if visual_gate["result"] != "PASS":
        raise ValueError("r5 campaign requires the r4 visual parity prerequisite PASS")
    ...
    if stage_gate["result"] != "PASS":
        raise ValueError("r5 campaign requires the scripted stage contract PASS")
    ...
    warp_contract = ResolvedActionWarpContractR5.from_config(
        bundle_dir / "config_snapshot.yaml"
    )
    receipts = []
            ...
            json.dumps(
                {
                    ...
                    "min_physical_base_command_norm": receipt[
                        "action_warp_diagnostics"
                    ]["physical_base_command_norm_first3"]["min"],
                },
                sort_keys=True,
            ),
            flush=True,
    ...
    cases_base_still_step = sum(
        item["action_warp_diagnostics"]["physical_base_command_norm_first3"]["min"]
        <= 0.1
        for item in receipts
    )
    if cases_stage1 == 0 and cases_base_still_step == 0:
        typed_conclusion = (
            "IN_CONTRACT_0_OF_8_BASE_STILL_STUDENT_UNDER_GAP_EVIDENCE"
        )
        p2_disposition = "PRE_AUTHORIZED_PROBE_REQUIRED"
    elif cases_stage1 == 0:
        typed_conclusion = "IN_CONTRACT_BASE_STILL_COMMAND_WITHOUT_STAGE1_TRANSITION"
        p2_disposition = "PRE_AUTHORIZED_PROBE_REQUIRED"
    else:
        typed_conclusion = "IN_CONTRACT_STAGE1_TRANSITIONS_OBSERVED"
        p2_disposition = "NOT_TRIGGERED"
    campaign = {
        "schema": "doordog.sim2sim.paired_mujoco_campaign_receipt.r5.v1",
        "evidence_level": "E4_FULL_CAMPAIGN_E5_MUJOCO_INPUT",
        "result_classification": "VALID_WITH_WARNINGS",
        "typed_conclusion": typed_conclusion,
        "p2_discriminative_probe_disposition": p2_disposition,
        "input_status": "MUJOCO_R5_CAMPAIGN_COMPLETE_ISAAC_PAIRED_TRACE_PENDING",
        "manifest_id": manifest["manifest_id"],
        "manifest": str(manifest_path),
        "manifest_case_seed_initial_state_unchanged_for_r5": True,
        "manifest_runtime_metadata_supersession": {
            ...
            "reason": "owner r3/r4/r5 adjudications; case material itself remains unchanged",
        },
        "policy_bundle": str(bundle_dir),
        "standing_vitals_gate": str(standing_gate_path),
        "action_warp_contract": str(action_warp_receipt_path),
        ...
        "action_warp_summary": {
            "base_clip_low": list(warp_contract.base_low),
            "base_clip_high": list(warp_contract.base_high),
            "base_command_clip_steps_by_axis_total": np.sum(
                [
                    item["action_warp_diagnostics"]["base_command_clip_steps_by_axis"]
                    for item in receipts
                ],
                axis=0,
            ).tolist(),
            "base_command_at_cap_steps_by_axis_total": np.sum(
                [
                    item["action_warp_diagnostics"]["base_command_at_cap_steps_by_axis"]
                    for item in receipts
                ],
                axis=0,
            ).tolist(),
            "final_20d_action_clip_steps_total": sum(
                item["action_warp_diagnostics"]["final_20d_action_clip_steps"]
                for item in receipts
            ),
            "min_physical_base_command_norm_by_case": {
                item["case_id"]: item["action_warp_diagnostics"][
        ...
        "stage_summary": {
            ...
            "cases_reaching_stage1": cases_stage1,
            ...
            "cases_with_min_physical_base_command_norm_le_0_1": cases_base_still_step,
        },
        "task_summary": {
        ...
        "prior_campaign_status": {
            ...
            "r4_campaign": "INVALID_PIPELINE_SUPERSEDED_BY_R5",
            "r4_behavior_claims": "VOID",
            "r4_preserved_valid_items": (
                "stage contract structure; true 100/45 native-position surface and standing "
                "gate; door scene/manifest/seeds/paired schema; visual visibility structure"
            ),
            "meaning": "owner-accepted r2/r3 classifications and the r4 owner adjudication are retained verbatim",
        },
        "producer_identity": {
            "git_commit_before_phase_commit": commit,
            "path": "gr00t/rl/sim2sim/cli/run_paired_mujoco_campaign_r5.py",
        },
        "warnings": [

"""Frozen bilateral acquisition actor with a shared canonical Stage3 head."""
...
class PullV6BilateralStage3CanonicalActor(PullV6PopulationOutputActor):
    """Train one mirror-canonical controller without reading recurrent parent state."""

    _OBS_DIM = 135
    # LeggedRobotBase concatenates actor observation components by sorted key.
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        if self.running_mean_std is None:
            raise RuntimeError("Bilateral canonical actor requires a frozen 135-D RMS.")
        if self.memory.rnn.input_size != self._OBS_DIM:
            raise RuntimeError("Bilateral canonical actor requires a 135-D parent carrier.")
        with torch.random.fork_rng():
            torch.manual_seed(0)
            self.bilateral_stage3_canonical_head = nn.Sequential(
                nn.Linear(58, 256),
                nn.SiLU(),
                nn.Linear(256, 256),
                nn.SiLU(),
        ...
        nn.init.zeros_(self.bilateral_stage3_canonical_head[4].weight)
        nn.init.zeros_(self.bilateral_stage3_canonical_head[4].bias)
        self._h13_clamp_noise_std = self.clamp_noise_std
        self.clamp_noise_std = False

    @staticmethod
    def _require_onehot(value: torch.Tensor, name: str) -> None:
        if not value.is_floating_point():
            raise RuntimeError(f"{name} must be floating point.")
        ...
        if not torch.all(binary):
            bad = value[~binary]
            raise RuntimeError(
                f"{name} must contain only zero/one values; "
                f"bad_min={bad.min().item()}, bad_max={bad.max().item()}."
            )
        if not torch.all(torch.abs(value.sum(dim=-1) - 1.0) <= tolerance):
    ...
    def _canonical_features(
        ...
        if current_obs.shape[-1] != self._OBS_DIM:
            raise RuntimeError(
                f"Bilateral canonical actor requires 135-D current_obs; got {current_obs.shape[-1]}."
            )
        raw = self.running_mean_std(current_obs, unnorm=True)
        if raw.shape != current_obs.shape:
            raise RuntimeError("RMS unnormalization changed the current_obs shape.")
        ...
        try:
            ...
            flattened = raw.reshape(-1, self._OBS_DIM)
            tolerance = 1.0e-4
            pair_candidates = []
            stage_candidates = []
            for start in range(self._OBS_DIM - 1):
                value = flattened[:, start : start + 2]
                binary = (torch.abs(value) <= tolerance) | (
                    torch.abs(value - 1.0) <= tolerance
                )
                if torch.all(binary) and torch.all(
                    torch.abs(value.sum(dim=-1) - 1.0) <= tolerance
                ):
                    pair_candidates.append(start)
            for start in range(self._OBS_DIM - 5):
                value = flattened[:, start : start + 6]
                binary = (torch.abs(value) <= tolerance) | (
                    torch.abs(value - 1.0) <= tolerance
                )
                if torch.all(binary) and torch.all(
                    ...
                    stage_candidates.append(start)
            raise RuntimeError(
                f"{exc} onehot_pair_candidates={pair_candidates}, "
                f"onehot6_candidates={stage_candidates}."
            ) from exc
        ...
        mirror = torch.where(mirror > 0.0, torch.ones_like(mirror), -torch.ones_like(mirror))

        relative_door = raw[..., self._RELATIVE_DOOR]
        handle_pose = raw[..., self._HANDLE_POSE]
        canonical_metadata = torch.cat((metadata[..., :5], metadata[..., 7:8]), dim=-1)
        ...
        features = torch.cat(
            (
                ...
                torch.cat(
                    (
                        self._polar(relative_door[..., :3], mirror),
                        self._polar(relative_door[..., 3:6], mirror),
                        self._polar(relative_door[..., 6:9], mirror),
                    ),
                    dim=-1,
                ),
                ...
                torch.cat(
                    (
                        self._polar(handle_pose[..., :3], mirror),
                        self._polar(handle_pose[..., 3:6], mirror),
                        self._polar(handle_pose[..., 6:9], mirror),
                    ),
                    dim=-1,
                ),
                canonical_metadata,
        ...
        if features.shape[-1] != 58:
            raise RuntimeError("Bilateral canonical feature contract must be 58-D.")
    ...
    def _mean_from_memory(
        ...
        **kwargs,
        ...
        head = self.bilateral_stage3_canonical_head(features)
        updated = result.clone()
        canonical_base = head[..., :3]
        decanonical_base = torch.stack(
            (canonical_base[..., 0], mirror * canonical_base[..., 1], mirror * canonical_base[..., 2]),
            dim=-1,
        )
        updated[..., :3] = torch.where(stage3.unsqueeze(-1), decanonical_base, result[..., :3])
        updated[..., 5:11] = torch.where(stage3.unsqueeze(-1), head[..., 3:], result[..., 5:11])
        return updated

"""Frozen pull-v6 carrier with a D-only post-release action residual."""
...
class PullV6PostReleaseActor(PullV6ReleaseModeActor):
    """Train a post-release residual while preserving the r6ag release carrier."""
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.post_release_d_head = nn.Linear(256, len(self._D_ACTION_INDICES))
        nn.init.zeros_(self.post_release_d_head.weight)
        nn.init.zeros_(self.post_release_d_head.bias)
    ...
    def _apply_post_release_residual(
        self, mean: torch.Tensor, memory_out: torch.Tensor, release_mode: torch.Tensor
    ) -> torch.Tensor:
        mean = mean.clone()
        d_mask = release_mode[..., 1].unsqueeze(-1)
        mean[..., self._D_ACTION_INDICES] += d_mask * self.post_release_d_head(memory_out)
        return mean

    def _mean_from_memory(self, memory_out: torch.Tensor, release_mode: torch.Tensor, **kwargs) -> torch.Tensor:
        mean = self._apply_release_mode_override(self.actor_module(memory_out, **kwargs), release_mode)
        return self._apply_post_release_residual(mean, memory_out, release_mode)
        ...
        **kwargs,
        ...
        if len(input_obs.shape) == 2:
            ...
            if masks is not None and original_dones is not None:
                ...
                if self.training:
                    raise RuntimeError(
                        "PullV6PostReleaseActor: masks and original_dones must be provided during training!"
                    )
        ...
        return self._mean_from_memory(memory_out, release_mode, **kwargs)
    ...
    def rollout(self, obs_dict, episode_attnmask=None, cur_dones=None, **kwargs):
        ...
        mean = self._mean_from_memory(memory_out, release_mode, **kwargs)

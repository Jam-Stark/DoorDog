"""Frozen pull-v6 recurrent carrier with mode-gated gripper means."""
...
class PullV6ReleaseModeActor(RecurrentActor):
    """Train only the ready/released gripper means on a frozen r6ad carrier."""

    def __init__(self, *args, **kwargs) -> None:
        super().__init__(*args, **kwargs)
        for parameter in self.parameters():
            parameter.requires_grad_(False)
        self.release_mode_gripper_mean_override = nn.Parameter(torch.zeros(2))
    ...
    def _apply_release_mode_override(
        self, mean: torch.Tensor, release_mode: torch.Tensor
    ) -> torch.Tensor:
        mean = mean.clone()
        gripper_mean = mean[..., 11]
        gripper_mean = torch.where(
            release_mode[..., 0] == 1,
            self.release_mode_gripper_mean_override[0],
            gripper_mean,
        )
        mean[..., 11] = torch.where(
            release_mode[..., 1] == 1,
            self.release_mode_gripper_mean_override[1],
            gripper_mean,
        )
        return mean
    ...
    def forward(
        ...
        **kwargs,
        ...
        return self._apply_release_mode_override(mean, release_mode)
    ...
    def act_inference(self, obs_dict, episode_attnmask=None, cur_dones=None, **kwargs):
        ...
        self.steps += 1
        return self._apply_release_mode_override(self.actor_module(memory_out, **kwargs), release_mode)

...
- [base-v28-camera-aware-rebaseline/description.md](base-v28-camera-aware-rebaseline/description.md): 2026-09-12 — G0 launch门通过：D37数值确认、PPO5batch、26字段接线完成；seed282无随机暴露，X24/X25保留；§9.7首commit完成且未push，G1尚未启动。
...
- [base-v26-scratch-bilateral-teacher/description.md](base-v26-scratch-bilateral-teacher/description.md): 2026-08-31 20:05 HKT — v26-6 Wave A 已把 v26 全程的 unlatch 阻塞定位为夹爪执行能力回退：v26 从未覆盖 `dof_effort_limit_list`，沿用 arm_j7/j8 `10/10 N` 与 M39 关闭，而 v18–v25/pull 全部是 `45/45 N` + `1300/32` + M39。eval-only 单因素 A/B（bit-exact 对照）typed route `GRIPPER_CAPACITY_CONFIRMED`：RIGHT `handle≥0.3` `16/64→48/64`，`drvF>1.6` 分层 `1/43→28/43`。v26-3 的 `ACTUATOR_CAPACITY_NOT_CAUSAL_AT_TESTED_RANGE` 因是 10 N 下训练的 eval-only ladder 且未同步放开 over-force/M39 而不成立。Stage4/goal 仍为 0，剩余阻塞是 Stage2→3 `-0.093/step` 收入悬崖与 `hinge<0.1` 的 unlatch 墙。历史 v26-4 R2 结论：2026-08-29 06:48 HKT — v26-4 R2已关闭为`CANONICALIZATION_NOT_SUPPORTED`。K以修正后的几何目标通过FK mirror与冻结网格证明`BILATERAL_ASYMMETRIC_IN_ACTION_OFFSET`，supersede R1 defective-target arm_j4结论；C canonical identity为CPU/static PASS、C1 runtime smoke PASS；四格formal train与32组exact64 bilateral eval完成，但step750 C1未通过预注册bands且seed1未达三指标strict improvement。orientation audit确认active A2 frame-transformer仍有side-independent target offsets，作为v26-5输入。无hardware evidence，Teacher/Student handoff与G7 binding不变。

...
- [push-open-door-optimization/description.md](push-open-door-optimization/description.md): 2026-07-16 00:29 HKT — 从 `base_v9` 起独立负责 A2+Piper full-stage 推门/开门 RL optimization；当前是 user-approved `base_v12` current-source、v10_A-style scratch 2×2 config anchor（A/B/C/D，global cap3000/save250），训练/eval 尚未启动。`base_v10_D` 仅保留为 behavioral reference，不是 v12 winner。该 entry 亦规定 future A2 eval 必须完整 co-locate 于 `logs_eval/base_vN/<eval-run>/`；默认 qualitative render 随机选2个 env、每个3个 camera，共6个视频。

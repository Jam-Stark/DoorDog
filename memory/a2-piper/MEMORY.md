...
- [push-open-door-optimization/description.md](push-open-door-optimization/description.md): 2026-07-17 20:05 HKT — `v13` diagnosis 已归档：A3000 为 `0/16 goal`、`16/16 stage4`、`4/16 stage5`；`base_v13_1_main` 已完成 M11–M14/config/validation/r3 smoke，training-ready，但正式 `2000`-batch training、matched eval 与render均未启动。后续按每250 batch监控并对checkpoint执行matched eval/render。
...
- [phase2-student-distillation-a2-piper/description.md](phase2-student-distillation-a2-piper/description.md): 2026-08-12 05:26 HKT — fixed-G2 C-B2H v19 GRPO finetune/eval complete：best step10 512 集 `467/512 = 91.21%`，较 `459/512` 提升 `1.56 pp`；Stage2 failures `35→23`，但 Stage0 `15→20`、Stage4 `1→2`，故 verdict 为 §6 `88–93%` edge，不是 closeout。v1 解冻编码器需 separately approved scope；无 determinism/generalization/deployment/lifecycle/physical-camera PASS。
- [phase3-student-bootstrapping/description.md](phase3-student-bootstrapping/description.md): 2026-08-12 05:26 HKT — A2 v19 actor-only recurrent GRPO implementation complete；GPU2/3 run 在 step40 early stop，四个 seed0 gates 均 `16/16`，best step10 quick `45/48`、same-seed scale `467/512`，最终为 edge verdict。

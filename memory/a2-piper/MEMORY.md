...
- [push-open-door-optimization/description.md](push-open-door-optimization/description.md): 2026-07-16 22:39 HKT — `v13_pre` 已以16/16 stage3 entry证实 control-step gate；`v13_A` main 与 `v13_B` gate-only 的 M2–M6/M9 code/config 已 training-ready，78 tests/compose/static checks 与两组4-rank `64 env/rank × 50` concurrent smoke、step50 checkpoint CPU load PASS。正式4-rank训练尚未启动；每组用 `1024 env/rank`，global batch4096，A/B 分别 cap3000/1500，future eval 继续 co-locate 于 `logs_eval/base_vN/<eval-run>/`。

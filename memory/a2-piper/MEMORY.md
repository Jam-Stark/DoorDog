...
- [phase2-student-distillation-a2-piper/description.md](phase2-student-distillation-a2-piper/description.md): 2026-08-08 20:03 HKT — ToeOut6/pitch−50° step8000 的 current-worktree formal diagnosis 完成：Student 三 seed 为 `29/48` goal，失败主要为 near-handle Stage0 dead-zone；强制 pure G2 Teacher 在同一 seed0/case 上复现完全相同的失败集合，故不应先归因于 Student/C-B2H 单独的 distillation gap。旧 c18/10k 结果保留为历史 context。下一步先诊断/改善 current-worktree G2 Teacher 的 Stage0 行为；如需环境归因，先扩充 selection 的完整 door customData。

...
- [push-open-door-optimization/description.md](push-open-door-optimization/description.md): 2026-07-16 21:09 HKT — `base_v12` 已完成并定位 5-physics-frame gate 时间尺度错误；`v13_pre` 已实施 M1 `control_streak/K=5` 并对 v12_C step3000 做 matched runtime，16/16 进入 stage3，首过 stage2 control-step min/median/max `16/20.5/27`，根因 A runtime 证实。当前下一步是尚未启动的 v13_A main 与 v13_B gate-only；future A2 eval 继续完整 co-locate 于 `logs_eval/base_vN/<eval-run>/`。

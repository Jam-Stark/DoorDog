...
- [pull-open-door-task/description.md](pull-open-door-task/description.md): 2026-08-14 22:35 HKT — pull-v5.1 repair closed under G3/G11. F5 load-only is ACTUAL; P2 paired K25 improved `3/16→16/16` with 13 favorable discordants and exact `p=0.0001220703125`, selecting release persistence as binding while E6/frame passage remained zero. Source-A metadata/delayed capture produced a 191-row `PASS_G8_PURE_A` bank with all closer buckets. P1 four-primitive anchor remained waypoint/solvable `64/64` but yaw `32/64` across all three G3 corrections, so door buckets/G2/P3/P4/dual-source eval are NOT_RUN and stopping condition is unmet. Formal review remains one FAIL wave with targeted runtime validation and no second reviewer PASS claim.

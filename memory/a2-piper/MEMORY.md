...
- [push-open-door-optimization/description.md](push-open-door-optimization/description.md): 2026-07-15 21:18 HKT — 从 `base_v9` 起独立负责 A2+Piper full-stage 推门/开门 RL optimization；`base_v11_repair_r1` 的 matched eval 没有 promotable checkpoint 或 causal root cause；下一步仅为 approval-gated、stage3-exposed 的 `base_v11_repair_r2` single-variable intervention。该 entry 亦规定 future A2 eval 必须完整 co-locate 于 `logs_eval/base_vN/<eval-run>/`；默认 qualitative render 随机选2个 env、每个3个 camera，共6个视频。

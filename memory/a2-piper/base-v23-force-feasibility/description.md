...
- R257/R258 historical start state：R256 code review `PASS`，R257 CPU REDUCE runtime QA 一次 `rc0` 后写入 formal-admission lock。当时 G1/G3 只有 launch evidence；后续 natural closure 由独立 cell/slice receipts 和 A1 Route-A evidence 覆盖，不能从旧 launch record 单独推导。
- A1 runtime closure：G1/G3/G5/G7 均 `FORMAL_CELL_COMPLETE`，natural completion、`return_code=0`、trainer global step `2500` 且 final step-2500 checkpoint present。A1-D0、A1-D1 和 A1 subwave receipts 均 seal natural completion；Route-A A1 另以 exact40/640 evidence seal mechanical selection。
...
- A1-D0 G1 cell: `logs_rl/launchers/base_v23/seed0/G1/cell_record.json`
- A1-D0 G3 cell: `logs_rl/launchers/base_v23/seed0/G3/cell_record.json`
- A1-D0 slice: `logs_rl/launchers/base_v23/seed0/A1-D0/slice_record.json`

...
Status at 2026-08-08 23:30 HKT: `TRAINING_PASS; R14 resolved; base_v16_B Gemini 335L stage1–5 sweep, Scheme C, C-B2 and TOEIN20 camera eval complete; C-B v16 Student one-update and GPU7 pilot complete; C-B2H exact64 formal 10000-step checkpoint complete; old Student-only eval quality FAIL/poor; ToeOut6/pitch−50° step8000 multi-seed Student baseline and true-Teacher Stage0 diagnostic corrected; lifecycle unresolved`。
...
- 以至少两个额外 seed 复跑 true Teacher Stage0 diagnostic，保留同一 Stage0 trace 与完整 17-key door `customData`；随后比较六个 seed0 failures 与 nearby successes 的 directional root-to-staging vector 和 movement-relevant base action。旧 `controller=teacher` lane 实为 Student rollout，不能复用为 Teacher evidence；不要先盲目 Student retraining 或 global threshold relaxation。Recurrent ONNX/export、final policy-quality improvement 与 open-door success evaluation 仍需独立 scope。
...
- 2026-08-08 23:30 HKT - ToeOut6/pitch−50° step8000 Student 三 seed formal baseline 保持 `29/48` goal；校正后的 true Teacher seed0 为 `10/16`，Student seed0 为 `8/16`，Teacher-only successes 是 env `2,9`。六个 true Teacher failures 均仅未通过 staging-distance gate，17-key `customData` 在单 seed n=16 下未分离 failure/success。先执行 multi-seed true Teacher replication 和 directional root-to-staging/action analysis，不盲目重训 Student 或放宽全局阈值。

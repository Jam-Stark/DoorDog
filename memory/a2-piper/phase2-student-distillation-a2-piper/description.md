...
Status at 2026-08-10 19:39 HKT: `TRAINING_PASS / R14_RESOLVED / V16B_335L_STAGE1_5_SWEEP_COMPLETE / SCHEME_C_RUNTIME_PASS_VISIBILITY_PARTIAL / SCHEME_C_B_LOWPROFILE_RUNTIME_SMOKE_PASS / V16_CB_STUDENT_ONE_UPDATE_PASS / V16_CB_GPU7_CAPACITY_STABILITY_PASS / C_B2_SENSORS_RUNTIME_PASS_PANORAMA_VISUAL_FAIL / C_B2_TOEIN20_RENDER_COMPLETE_PANORAMA_VISUAL_FAIL / C_B2H_DUALRAW_64E_ADMISSION_PASS / C_B2H_FORMAL_10K_CHECKPOINT_COMPLETE / C_B2H_STUDENT_EVAL_RUNTIME_PASS_POLICY_QUALITY_FAIL / C_B2H_RENDER_BEST_ONLY_COMPLETE / C_B2H_TOEOUT6_TRUE_TEACHER_STAGE0_DIAG_COMPLETE / C_B2H_TOEOUT6_G2_STAGE0_CONTRACT_RUNTIME_PASS / C_B2H_G2_MULTISEED_STAGE2_DIAG_COMPLETE / C_B2H_LARGESCALE_CAMERA_EVAL_COMPLETE / C_B2H_LIFECYCLE_UNRESOLVED`。DoorDog-A2_Piper 在本 entry 中只按 A2+Piper route 处理；原 one-update、C-B v16 one-update、GPU7 10-batch pilot 与 C-B2H 64-env admission 都有真实 training update 证据，不是仅 `STATIC PASS`。旧 c18/10k seed0 quality FAIL 保留为历史 context。ToeOut6/pitch−50° fixed-G2 true Teacher seed0/1 为 `32/32`；formal pure Student seed0/1/2 为 `13/16`、`16/16`、`13/16`，合计 `42/48 = 87.5%`。seed0 failures `{4,6,9}` 与 seed2 `{8,12,14}` 无交集，matched Stage2 diagnosis 已关闭 contract-drift/handle-visibility 假设，支持 intermittent Student bilateral contact/squeeze-continuity robustness。2026-08-10 large-scale eval 补齐 formal Student `459/512 = 89.6484375%`、true-action Teacher `255/256 = 99.609375%`、gap `9.9609375 pp`；它保留 Stage2 contact-continuity 为主信号，但 direct visual metrics 为 `0/768`，可见性归因是 `UNKNOWN/INCONCLUSIVE`。上述正式结果只覆盖指定 seed、16 env、每 env 一 episode；same-seed replay 仍会变化，不能外推为 determinism、general/deployment 或 physical-camera PASS。旧 `controller=teacher` 结果实际绕过 Teacher `policy_step` 并执行 Student rollout，故其 `8/16` 和“完全相同失败集合”结论无效且已 superseded。camera sweep、Scheme C 与 C-B2 验证仍只作 Teacher eval diagnostic。C-B2 两个 panorama仍因重影、撕裂和空洞保持视觉 FAIL。

Current delta at 2026-08-12 05:26 HKT: `A2_V19_GRPO_FINETUNE_EDGE_COMPLETE`。fixed-G2 C-B2H actor-only GRPO 在 step40 early stop，best step10 same-seed 512 为 `467/512 = 91.2109375%`，较 `459/512` 增加 8 case；Stage2 failures `35→23`，但 Stage0 `15→20`、Stage4 `1→2`，因此命中 §6 edge 而不是 closeout。详细实现、训练与验收见下方新 section 和 `scriptsFORhuman/C-B2H_v19_GRPO_Finetune_Report_20260812.md`。
...
## C-B2H Large-scale Camera Evaluation

2026-08-10 19:39 HKT，`C_B2H_LARGESCALE_CAMERA_EVAL_COMPLETE` 在 fixed-G2 contract 下完成：Student `32 seeds × 16 env = 512`，`459/512 = 89.6484375%`，Wilson95 `[0.8670780645, 0.9199854665]`；true-action Teacher `16 × 16 = 256`，`255/256 = 99.609375%`，Wilson95 `[0.9782090246, 0.9993101178]`；Teacher−Student gap 为 `9.9609375 pp`。true Teacher smoke contract 取 `TRLDistillTrainerA2BaseAPI.policy_step.gt_actions`，ratio `1`，`694/11104` env-actions exact action match，composed action `24D`，Student rollout calls `0`。

- Student 53 个 failures 全为 `stage_overtime`：Stage0/1/2/4 分别 `15/2/35/1`。Stage2 contact-continuity 仍是 dominant failure signal；较高 handle-height association 只属描述性关联，不能在没有 direct visual metrics 时归因于 perception。
- Student success 高于 `70%`，pipeline anomaly defense `NOT_TRIGGERED`。failure-set Jaccard 为 496 seed pairs 中 486 个 defined non-empty-union pairs、10 个 empty-union undefined/excluded，mean `0.06172839506172839`。17 个 customData fields 均保留，其中 4 个为 formal-exact overlap、13 个为 deterministic seeded-provenance joins。
- 直接视觉指标在 initial capture 加两次 retries 后达到 fail-fast retry limit，保持 `0/768` 与 JSON `null`；camera/perception attribution 是 `UNKNOWN/INCONCLUSIVE`，既不能说明 current camera sufficiency，也不能把它写成 false。预设 verdict 为 `(c) 混合 / 暂归类`：contact/kinematic evidence strong，而 visual share unresolved。本轮不默认开启 D435 depth 或增加 wrist camera；若有 separately approved perception ablation，应先评估已有 D435 depth。
- 结果见 `scriptsFORhuman/C-B2H_v19_LargeScale_Camera_Eval_Report_20260810.md` 与 `logs_eval/by_batch/cb2h_v19_toeout6_pitch50_largescale_camera_eval_20260810/summary/`。R4 end-to-end analyzer、C2 targeted `CODE_QUALITY` 与 sealed-artifact/resource QA 均 PASS；QA validation level 是 `NO_SIM_PASS`，formal shard coverage complete/no gaps，GPU4–7、tmux 与 process 已释放。没有 training，也没有 determinism、generalization、deployment、lifecycle 或 physical-camera PASS。

## A2 v19 GRPO Finetune Closure

2026-08-12 05:26 HKT，`A2_V19_GRPO_FINETUNE_EDGE_COMPLETE`：新增 actor-only recurrent GRPO trainer/config/GPU2-3 DDP launcher，只更新 Student LSTM/action head，冻结 vision/Head encoder/std；return 为 success 减 `λ_ar=0.5` action-rate cost，global group advantage 用 DDP 聚合，无 critic/GAE/reference KL。exact 128D latent 与 stepwise recurrent replay 保持 rollout/update 同源；success extraction 与 trajectory advantage broadcast 在 smoke 中通过。

- pilot 选 `std=0.08`、learning rate `3.75e-6`、32 env/rank；40 iteration 总纯 iteration 墙钟 `14755.14s`，zero-variance group 为 0。
- step10/20/30/40 seed0 gates 均 `16/16`，40/200 early stop；同分取最早 step10 为 best。quick 为 `45/48`，原 baseline `42/48`。
- seeds0–31、每 seed16 env 的 same-contract formal eval 为 `467/512 = 91.2109375%`；基线为 `459/512 = 89.6484375%`。45 failures 全为 `stage_overtime`，Stage0/1/2/4 为 `20/0/23/2`，对比 `15/2/35/1`。
- Stage2 contact-continuity signal 得到明显改善，但 Stage0/Stage4 回退意味着未满足“≥93% 且其他 stage 不增”。最终 verdict 是设计 §6 `88–93%` edge；v1 有限解冻视觉编码器只作 future separately approved 建议，未启动。
- natural Kit close 仍可能在 checkpoint/eval artifact 落盘后停滞；只终止自有 closing process。该 lifecycle fact 不否定 checkpoint/metrics/eval，也不构成 natural lifecycle PASS。
...
- 2026-08-10 19:39 HKT - Large-scale eval 已完成并归档：Student `459/512`、true-action Teacher `255/256`，Stage2 contact-continuity 仍是主失败信号；direct visual metrics `0/768` 使 camera/perception attribution 保持 `UNKNOWN/INCONCLUSIVE`。任何 direct-visibility capture repair、existing D435-depth ablation 或 wrist-camera decision 都是 separately approved future work；当前不默认启用 depth 或添加 wrist camera。targeted Stage2 DAgger/contact-continuity finetune 仍需新的 approved `HIGH_RISK` brief。
- 2026-08-12 05:26 HKT - v19 GRPO v0 已完成并得到 edge verdict；future v1 需 separately approved，目标是保留 Stage2 `35→23` 改善并消除 Stage0 `15→20` 回退。本轮不启动 v1。
## DONE Summary
...
- 2026-08-10 19:39 HKT - `C_B2H_LARGESCALE_CAMERA_EVAL_COMPLETE`: fixed-G2 large-scale eval completed Student `32×16 = 512` (`459/512 = 89.6484375%`, Wilson95 `[0.8670780645, 0.9199854665]`) and true-action Teacher `16×16 = 256` (`255/256 = 99.609375%`, Wilson95 `[0.9782090246, 0.9993101178]`), gap `9.9609375 pp`. Teacher used `TRLDistillTrainerA2BaseAPI.policy_step.gt_actions`, ratio `1`, exact match `694/11104`, `24D` composition, Student calls `0`. Student failures were Stage0/1/2/4 `15/2/35/1`, all `stage_overtime`; Stage2 contact-continuity remains dominant, while higher handle height is not perception-attributable without direct visual metrics. Student >70% defense was not triggered; Jaccard mean was `0.06172839506172839` across 486 defined pairs (10 empty-union pairs excluded); 17 customData fields retained as 4 formal-exact plus 13 deterministic seeded-provenance joins. Direct visual metrics remained `0/768`/null after the three-attempt fail-fast retry limit, so verdict `(c) 混合 / 暂归类` and camera/perception attribution are `UNKNOWN/INCONCLUSIVE`. Report: `scriptsFORhuman/C-B2H_v19_LargeScale_Camera_Eval_Report_20260810.md`; summaries: `logs_eval/by_batch/cb2h_v19_toeout6_pitch50_largescale_camera_eval_20260810/summary/`. R4 analyzer, C2 `CODE_QUALITY`, and sealed-artifact/resource QA PASS at `NO_SIM_PASS`; coverage complete/no gaps and resources released. No training, determinism, generalization, deployment, lifecycle, or physical-camera PASS.
- 2026-08-12 05:26 HKT - `A2_V19_GRPO_FINETUNE_EDGE_COMPLETE`: actor-only recurrent GRPO smoke/pilot/main/gates passed; step10 best quick was `45/48`, same-seed 512 was `467/512 = 91.2109375%` versus `459/512`, with Stage0/1/2/4 failures `20/0/23/2` versus `15/2/35/1`. Verdict is §6 edge, report is `scriptsFORhuman/C-B2H_v19_GRPO_Finetune_Report_20260812.md`, and v1 was not started.

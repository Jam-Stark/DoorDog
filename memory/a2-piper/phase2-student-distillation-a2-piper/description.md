...
Status at 2026-08-08 20:03 HKT: `TRAINING_PASS / R14_RESOLVED / V16B_335L_STAGE1_5_SWEEP_COMPLETE / SCHEME_C_RUNTIME_PASS_VISIBILITY_PARTIAL / SCHEME_C_B_LOWPROFILE_RUNTIME_SMOKE_PASS / V16_CB_STUDENT_ONE_UPDATE_PASS / V16_CB_GPU7_CAPACITY_STABILITY_PASS / C_B2_SENSORS_RUNTIME_PASS_PANORAMA_VISUAL_FAIL / C_B2_TOEIN20_RENDER_COMPLETE_PANORAMA_VISUAL_FAIL / C_B2H_DUALRAW_64E_ADMISSION_PASS / C_B2H_FORMAL_10K_CHECKPOINT_COMPLETE / C_B2H_STUDENT_EVAL_RUNTIME_PASS_POLICY_QUALITY_FAIL / C_B2H_RENDER_BEST_ONLY_COMPLETE / C_B2H_TOEOUT6_STAGE0_DIAG_COMPLETE / C_B2H_LIFECYCLE_UNRESOLVED`。DoorDog-A2_Piper 在本 entry 中只按 A2+Piper route 处理；原 one-update、C-B v16 one-update、GPU7 10-batch pilot 与 C-B2H 64-env admission 都有真实 training update 证据，不是仅 `STATIC PASS`。旧 c18/10k seed0 quality FAIL 保留为历史 context。当前 current-worktree ToeOut6/pitch−50° 8k Student 的三 seed formal baseline 为 `29/48` goal，失败主要为 Stage0 near-handle dead-zone；强制 pure G2 Teacher 在同一 seed0/16 case 重现 Student 的完全相同失败集合，故此边界不能先归因于 Student/C-B2H 单独的 distillation gap。三次同一 sealed case replay 都产生 outcome drift，故不声称 same-seed replay bitwise deterministic。camera sweep、Scheme C 与 C-B2 验证仍只作 Teacher eval diagnostic。C-B2 两个 panorama 仍因重影、撕裂和空洞保持视觉 FAIL。
...
2026-08-08 20:03 HKT，current-worktree、ToeOut6/pitch−50° 的 step8000 Student 通过 formal 16-env、one episode per env 的三 seed baseline：seed0 `8/16`、seed1 `12/16`、seed2 `9/16`，合计 `29/48` goal（`60.4%`）。终局为 Stage0 `16`、Stage2 `3`、Stage5 `29`；这不是 bitwise determinism claim。

- seed0 中 Student 的失败集合是 Stage0 env `0,2,4,5,7,12,15` 与 Stage2 env `9`。在同一 16 case/seed0、`ratio=1.0` 且强制 Teacher rollout 的对照中，G2 Teacher 同样为 `8/16`，且失败集合完全一致。因此 primary boundary 是 current-worktree G2 Teacher/任务的 Stage0 near-handle dead-zone，不应先归因于 Student 或 C-B2H 的单独 distillation gap。
- Student env0、env5、env12 的准确 case render 都在 episode 250 以 `stage_overtime` 结束：接近手柄后几乎停止，未建立接触。诊断视频在 `logs_eval/by_batch/cb2h_v19_toeout6_pitch50_diag_20260808/failure_renders_seed0/`；该证据说明停滞，而不证明单一视觉或动作根因。
- training 与 eval 都使用 `max_stage_time=[250,100,100,100,100,200]`，故当前证据排除“训练 Stage0 timeout 比 eval 更宽松”的 mismatch。
- current selection 只封存 handle height、weight、hinge max force、handle max force 四项；`DoorSpawnerCfg` 还可随机化 door width/height、handle width/geometry/hook/radius、hinge stiffness 等。因此这四列重叠不能排除环境几何影响，不能把 failure split 简化为与环境无关。
- `gr00t/rl/scripts/run_a2_toeout6_student_eval.py` 已具备 formal Teacher、multi-seed Student 与 validated arbitrary Student env render。后续先诊断/改善 G2 Teacher 的 Stage0 near-handle 行为；如需要 environment attribution，扩充 selection 的完整 door customData。当前不建议只靠盲目增加 Student iteration 来处理该 Teacher-shared boundary。
...
- 2026-08-08 20:03 HKT - ToeOut6/pitch−50° step8000 已完成三 seed Student baseline 与同 case/seed0 pure Teacher 对照。后续优先诊断/改善 current-worktree G2 Teacher 的 Stage0 near-handle dead-zone；若要做环境难度归因，先把完整 door customData 写入 selection。不要先以增加 Student iteration 作为默认修复。
## DONE Summary
...
- 2026-08-08 20:03 HKT - `C_B2H_TOEOUT6_STAGE0_DIAG_COMPLETE`: current-worktree ToeOut6/pitch−50° step8000 Student 三 seed formal baseline 为 `29/48` goal，终局 Stage0/Stage2/Stage5 为 `16/3/29`。seed0 pure G2 Teacher 对照为 `8/16`，与 Student seed0 具有完全相同的 Stage0 env `0,2,4,5,7,12,15` 和 Stage2 env `9` 失败集合。env0/env5/env12 render 重现 Stage0 overtime、near-handle 停滞与无接触；training/eval Stage timeout 一致。该证据优先指向 Teacher-shared Stage0 boundary，且 selection 的四个门参数不足以排除其他 door geometry randomization。

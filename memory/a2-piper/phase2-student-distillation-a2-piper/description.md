...
Status at 2026-08-08 23:30 HKT: `TRAINING_PASS / R14_RESOLVED / V16B_335L_STAGE1_5_SWEEP_COMPLETE / SCHEME_C_RUNTIME_PASS_VISIBILITY_PARTIAL / SCHEME_C_B_LOWPROFILE_RUNTIME_SMOKE_PASS / V16_CB_STUDENT_ONE_UPDATE_PASS / V16_CB_GPU7_CAPACITY_STABILITY_PASS / C_B2_SENSORS_RUNTIME_PASS_PANORAMA_VISUAL_FAIL / C_B2_TOEIN20_RENDER_COMPLETE_PANORAMA_VISUAL_FAIL / C_B2H_DUALRAW_64E_ADMISSION_PASS / C_B2H_FORMAL_10K_CHECKPOINT_COMPLETE / C_B2H_STUDENT_EVAL_RUNTIME_PASS_POLICY_QUALITY_FAIL / C_B2H_RENDER_BEST_ONLY_COMPLETE / C_B2H_TOEOUT6_TRUE_TEACHER_STAGE0_DIAG_COMPLETE / C_B2H_LIFECYCLE_UNRESOLVED`。DoorDog-A2_Piper 在本 entry 中只按 A2+Piper route 处理；原 one-update、C-B v16 one-update、GPU7 10-batch pilot 与 C-B2H 64-env admission 都有真实 training update 证据，不是仅 `STATIC PASS`。旧 c18/10k seed0 quality FAIL 保留为历史 context。current-worktree ToeOut6/pitch−50° 8k Student 的三 seed formal baseline 仍为 `29/48` goal；经 action-route proof 校正后的 true Teacher seed0 为 `10/16`，Student seed0 为 `8/16`，Teacher-only successes 是 env `2,9`，两者共享 Stage0 failures 为 env `0,4,5,7,12,15`。旧 `controller=teacher` 结果实际绕过 Teacher `policy_step` 并执行 Student rollout，故其 `8/16` 和“完全相同失败集合”结论无效且已 superseded。true Teacher 的六个失败均卡在 Stage0 staging-distance gate；不应把它们先归因于单独 Student distillation gap，也不应在单 seed 证据下做 global threshold relaxation。三次同一 sealed case replay 都产生 outcome drift，故不声称 same-seed replay bitwise deterministic。camera sweep、Scheme C 与 C-B2 验证仍只作 Teacher eval diagnostic。C-B2 两个 panorama 仍因重影、撕裂和空洞保持视觉 FAIL。
...
- `gr00t/rl/scripts/run_a2_toeout6_student_eval.py` 已具备 formal Teacher、multi-seed Student 与 validated arbitrary Student env render。下一步先以至少两个额外 seed 复制 true Teacher Stage0 trace/full `customData`，并对六个 failures 做 directional root-to-staging vector 与 movement-relevant base action 分析；不要盲目 Student retraining，也不要 global threshold relaxation。
...
- 2026-08-08 23:30 HKT - ToeOut6/pitch−50° step8000 true Teacher diagnostic 已校正旧 mislabeled Teacher lane：true Teacher 为 `10/16`、Student seed0 为 `8/16`，Teacher-only successes 为 env `2,9`；六个 shared Teacher Stage0 failures 仅差 staging-distance gate，17-key `customData` 尚未产生单一分离维度。后续进行 multi-seed true Teacher replication 和 directional root-to-staging/action analysis；不先盲目 Student retraining 或 global threshold relaxation。
## DONE Summary

...
- 2026-07-05 19:00 HKT - 下一步 full-stage debug 不要把 `arm_j7/j8 Kp/Kd=80/3` 当作主要嫌疑；`base_v3` tightened 对照已确认 same gain 能产生真实 contact/squeeze，但新 close-gate/progress tightened completion 会拒绝该类 grasp。后续应优先做 A/B rollback：保持 A2 strict route，恢复 `a2_stage2_completion_close_gate_required=false` 或重新设计 completion，使其接受真实 bilateral contact/squeeze 而不是要求固定 close-progress 形态。

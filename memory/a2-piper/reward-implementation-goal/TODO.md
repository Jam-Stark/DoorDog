...
- 2026-07-08 20:20 HKT - `base_v7` A/B 后续：A (`TCP=0.085, arm_j7/j8 effort=10`) 作为当前 full-stage baseline；不要默认恢复 40N effort。若要确认 base_v6 stage1 no-arm collapse 是否为组合效应，应单独 repeat C (`TCP=0.085, effort=40`)；但下一步优先级更高的是 stage3/4 diagnostics/reward：记录 stage3/4/5 gripper raw、j7/j8 pos、door hinge/handle joint、root yaw/roll/pitch、doorframe contact、target_root distance，并补 keep-close/contact-retention 与 through heading/clearance 约束。

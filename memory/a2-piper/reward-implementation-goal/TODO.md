...
- 2026-07-07 22:09 HKT - `base_v6` 训练/eval 后必须重点检查 TCP 后移 + `arm_j7/j8 effort_limit_sim=40N` 的组合效果：render 侧看 handle 是否从 finger tip 移到 finger mid-section，trace 侧看 `arm_body7` contact 是否增加、`both_contact/contact_stability/squeeze_window` 是否上升、`single_contact_arm_body8` 是否下降、是否出现 over-force / raw close saturation / `upper_dof_overspeed`。不要把结果误归因为单变量 effort cap；该 run 同时改变了 TCP geometry 与 effort limit。

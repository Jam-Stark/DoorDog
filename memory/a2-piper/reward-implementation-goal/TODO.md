...
- 2026-07-10 21:14 HKT - 由 user 运行 `base_v9_A/B/C/D` 四组正式 matched training：每组 2 processes、每 rank `num_envs=2048`（4096 total）、1000 batches，使用同一个 `base_v8 A` ckpt1000 做 `checkpoint_load_mode=policy_only` warm start，并严格对齐 seeds、training budget 与其余 config。收集四个 run dir / ckpt1000 后先做 scalar/trace eval，再给 winner 补同 seed rendering。`2048 env/GPU` 是参考 `4×1024` 命令的 2 倍 per-GPU load；如 OOM，先报告，不要单独改变某组 env count 破坏 matched comparison。原 scratch B near-gate reset / stage2 base-mask 路线未实施，保留为 historical deferred branch，不是当前 blocker。
- 2026-07-10 18:54 HKT - 首轮 2×2 固定 workspace-margin shaping；若 base-unlock 后仍持续出现 normalized arm margin `<0.05`、`arm_j6` soft-limit saturation 与 door progress plateau，第二轮再单独 A/B workspace-margin shaping。不要与 threshold/base factor 同轮混入，也不要默认启用 arm default-pose reward。

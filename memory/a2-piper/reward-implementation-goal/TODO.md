...
- 2026-07-08 14:20 HKT - 真正 `base_v6_40_effort_08TCP_offset` ckpt1000 eval 没进入 stage2，无法判断 `arm_j7/j8 effort_limit_sim=40N` 对 handle contact force 的效果。后续若要评估 TCP 后移 + 40N，必须使用能进入 stage2 的 later checkpoint 或专门 eval/diagnostic run；不要把 `logs_eval/full_stage_base_v5_no_reward_penalty_scale_last_render_eval2` 的 `arm_body8` 单侧 force/spike 当作 base_v6 evidence，因为该目录对应 `base_v5_no_reward_penalty_scale`，saved config 仍是 `arm_j7/j8 effort=10.0/10.0`。

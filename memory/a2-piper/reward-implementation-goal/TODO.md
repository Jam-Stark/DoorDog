...
- 2026-07-05 21:34 HKT - rollback short full-stage 600 eval 已完成；后续跑/检查 1000 checkpoint eval。若 1000 仍是 close gate 高、`negative_gripper_primitive=0`、contact/squeeze=0，则下一步重点转向 close-stage reward credit / arm-vs-base reach shaping，而不是 completion gate 或 gripper gain。
- 2026-07-05 22:04 HKT - base creep penalty 已进一步增强：`penalty_a2_stage1_stage2_base_forward_creep=-1.5`、deadband `0.05`、scale `0.10`。下一步 full-stage retrain/eval 检查 stage1/2 base-forward creep 是否下降、arm/TCP reach 是否增加、deterministic `negative_gripper_primitive` 是否从 0 恢复；同时观察是否过度抑制必要 micro-adjust 或导致 stage1->2 cadence regression。

...
- 2026-07-11 16:44 HKT - 下一步只做 read-only exact geometry diagnosis：逐项核对 `FrameTransformer` source/target、`grasp_target` prim pose、`arm_body7/8` finger colliders、handle collider/contact surfaces、finger closing axis、closed aperture 与 bilateral reachability。必须把 source/target transform、collider geometry、contact surface 和夹爪实际闭合包络对到同一坐标系；geometry root cause 当前仍是 hypothesis。诊断完成前不提出 v10 code/plan，也不启动长训练。

...
- 2026-07-04 22:56 HKT - A2 stage2 completion 已收紧为 reversible experiment（close_gate + stable_close + close_progress_min gates，flag `a2_stage2_completion_close_gate_required`）。下一步用旧 `ckpt6000`（patched `termination_level=1.0`）跑 IsaacSim eval 验证 stage2→3 不再被 contact spike 误触发；如果 fresh full-stage retrain 效果变好/变差，要考虑 A/B 恢复 `a2_stage2_completion_close_gate_required=false` 而不是忘掉这个变量。

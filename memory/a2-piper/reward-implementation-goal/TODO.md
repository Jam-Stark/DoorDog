...
- 2026-07-06 21:00 HKT - `base_v3` ckpt1000 no-render completion A/B 已完成。下一步不要把 `stage2_grasp_contact_history_length=3` 直接设为默认；它会 16/16 进入 stage3 但推进太早、terminal 多为 single-contact。优先考虑 `a2_stage2_contact_force_threshold=0.8`，先跑 2-env render eval 确认没有明显视觉假推进，再决定是否提交默认 config 改动。

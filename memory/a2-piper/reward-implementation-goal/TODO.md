...
- 2026-07-05 19:14 HKT - 跑 600/1000 iteration short full-stage train/eval，验证 `a2_stage2_completion_close_gate_required=false` A/B rollback 后：strict route 是否仍生效、`a2_stage2_grasp_complete_frac` / `to3_advance_frac` 是否恢复、`both_contact/sufficient_squeeze/opposite_squeeze` 是否真实持续，以及是否重新出现短暂 contact spike false-success。

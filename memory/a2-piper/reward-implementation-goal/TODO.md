...
- 2026-07-06 20:35 HKT - `base_v3` ckpt1000 no-render eval 已验证 dense reward 改善 close-stage behavior，但 stage2 completion 仍卡在 `arm_body7` force 与 5-frame bilateral contact stability。下一步用同一 checkpoint 做 reversible no-render A/B：`stage2_grasp_contact_history_length=3`（keep `contact_force_threshold=1.0`）和 `a2_stage2_contact_force_threshold=0.8`（keep history 5），再决定是否改默认 completion。

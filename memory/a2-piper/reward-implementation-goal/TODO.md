...
- 2026-07-06 15:45 HKT - A2 stage2 bilateral contact/squeeze dense reward 与 diagnostics 已完成；下一步 runtime TODO 是 retrain/eval，验证新 dense reward 是否提高 `both_contact` / `opposite_squeeze` / squeeze force window / contact stability，减少 single-contact、over-force 与 gripper raw sign flip，并确认 strict route 下 stage2→3 不再依赖旧 spike。
...
- 2026-07-06 15:45 HKT - `restrictPre-Grasp_v2` 后提出的 bilateral squeeze/contact-force、force stability 与 over-force reward work 已在 main env 完成；continuous aperture primitive 与 primitive rate/hysteresis 仍 deferred。下一步不要继续把主要 blocker 归因到 grasp target 位置，应通过 retrain/eval 验证 dense reward 是否改善 close-stage contact behavior。

...
- 2026-07-09 12:33 HKT - `base_v8 A'` / B runtime follow-up：先 train/eval A' release-after-open（stage3->4 threshold 回到 `0.174533`，stage4 release handle，arm default shaping 默认 disabled），再和 B hold-handle route 对比 `stage2_5_step_trace.json`（同时保留 legacy `stage2_step_trace.json`）。重点检查 stage4 gripper raw、arm 是否仍追 handle、doorframe contact、root yaw/roll/pitch、base 是否正着 through、`goal_reached`，再决定 release/hold 哪条进入默认 route。

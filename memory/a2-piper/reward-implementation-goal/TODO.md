...
- 2026-07-04 22:26 HKT - Full-stage strict-route runtime 验收发现旧 `ckpt6000` 仍可 2/2 complete；下一步如果要精确区分 bypass route 与 grasp-predicate-too-loose，需要补 eval `to_log` scalar dump 或短 retrain/W&B，重点看 `a2_stage1_to2_bypass_blocked_frac`、`a2_stage2_to3_bypass_blocked_frac`、`a2_stage2_grasp_complete_frac` 与 close/contact/squeeze stability。

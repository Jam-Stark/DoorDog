...
- 2026-07-11 22:19 HKT - Hold-oracle tooling 与 G1/G2/G3 matrix 已完成，但 geometry diagnosis 只 partial completion。下一步必须在同一坐标系中确认 `arm_body7/8` convexHull 与 `handle_inside` Capsule 的 exact inner contact surface、finger closing axis / closed aperture，并验证 current `grasp_target` / TCP `0.085` 的 bilateral reachability。Current effort baseline 是 `arm_j7/j8 effort_limit_sim=10/10`；不先改 Kp/friction，不把已证明 branch 可达但仍 center-timeout 的 base relief 当作 geometry fix。

...
- 2026-07-04 22:34 HKT - Full-stage strict-route scalar eval 已确认旧 `ckpt6000` 不是 door-open bypass route，而是 stage2 `grasp_complete` spike route；下一步先收紧或重设计 stage2 grasp completion / contact stability（例如要求跨 RL steps 稳定、加入 gripper aperture/handle-relative stability、或把 ContactSensor history 语义从 physics-sample spike 改成 env-step persistence），再决定是否 fresh full-stage retrain。

...
- 2026-07-10 16:24 HKT - `base_v8 B`-only `P0` follow-up：`D0` 已完成并确认 scratch ckpt1000 16/16 卡 stage2 open-gripper/TCP-retreat local optimum，尚未进入 stage3/4。下一步在任何 code 修改前向 user 提交 `D1/D2` 方案：物理一致的 near-gate stage2 reset 下对比 base free / stage2 base masked，同时补 stage1->2 前后 reward、arm/base action、close-gate 与 recurrent history diagnostics。若 near-gate policy 能 close/contact，再进入 scratch stage2 reward `aggregation x approach-depth std` ablation；若仍持续 open，则优先修 close credit/action semantics。长训练继续保持需 user 预先确认。

...
- 2026-07-03 15:13 HKT - `base_v2` 已验证 `arm_j7/j8 Kp/Kd 80/3` 会导致 open-gripper local optimum；下一步 reward/primitive work 应先处理 gripper primitive / close-stage reward credit assignment 与 stage1/base-creep 约束。更强 gripper dynamics 或 `30N` effort cap 应放在 close behavior 稳定后的 curriculum/fine-tune。

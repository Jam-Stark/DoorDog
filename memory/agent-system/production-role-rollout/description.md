...
- Ordinary production roles 可在默认 `STANDARD_PATH` 下完整 routing；planner/goal/IsaacLab/risk/memory lanes 均按具体 trigger 调用，不再固定组成两轮全量 review。`HIGH_RISK_PATH` role wave 必须绑定 user 已同意的 exact brief。
- Direct registration 是 user 明确决定；effective child metadata 未暴露时保持 `UNKNOWN/INCONCLUSIVE`，不允许 false runtime/model PASS。
- Deep registered-but-dormant；Standard 与 approved High 都可提交 exact brief 申请，但每次 invocation 必须重新批准，High consent 不构成 Deep approval；write-safety runtime eval 与 hook implementation 也需要 separate approval。
...
- 2026-07-13 17:23 HKT - 对九个 non-Deep role 做 route-aware contract eval：Standard authorization、High brief consent、triggered review、scoped candidate verification 与 durable-memory-only curator；完成前该 behavior 保持 `NOT_RUN`。
...
- 2026-07-13 17:23 HKT - Production role contracts 适配三级 route：Standard 可完整调用 ordinary roles，High 需要 exact consent evidence，Deep 在两条 delegated route 中仍逐次审批；reviewer 改为 assigned-path/candidate verification，memory curator 只接受 non-mechanical durable delta 与 triggered-lane PASS。TOML/static consistency PASS，route-aware runtime behavior `NOT_RUN`。

...
- DONE 2026-06-29 21:00 HKT: Stage3-5 transition conditions 核查完成。`_stage_3_reward_condition()` / `_stage_3_to_4_advance_condition()` / `_stage_4_reward_condition()` / `_stage_4_to_5_advance_condition()` / `_stage_5_reward_condition()` / `_stage_5_to_complete_condition()` 全部与 G1 origin 逐字节一致，不需要改 code。只依赖 door joint state（hinge/handle threshold，robot-agnostic）和 robot root x 位置（A2 trunk 与 G1 pelvis x 语义相同）。剩余 threshold tuning 等 smoke 暴露。
...
| `_stage_2_to_complete_condition()` | handle contact count `>=4`，按 selected hand 判断 grasped | A2 `_use_a2_base` path 已重写为 Piper 双侧 grasp completion + **5-step contact history gate**（`stage2_grasp_contact_history_length: 5`）：`forces_w_history` 来自 handle-specific `arm_body7` / `arm_body8` contact sensor history，shape `(num_envs, H, 2, 3)`；force 按 `_reward_grasp()` 同样方式旋到 Piper source/TCP frame；要求最近 H 个 history samples 都满足两侧 force norm `>1.0`、两侧 source local `abs(Y)>0.5`、两侧 `Y` force sign opposite；且 `actual_time_in_stage_buf >= H-1`（进入 stage2 后至少等 H 步才可能 complete）；stage_buf 必须 == STAGE_GRASP | PASS static implementation | contact history gate 防止瞬时 contact spike 误触发 stage2→3 advance。smoke 看 H=5 是否合适、contact magnitude、force threshold。 |
...
| `_stage_3_reward_condition()` | keep grasping: `_stage_2_to_3_advance_condition()` AND `_stage_2_reward_condition()` | A2 继承同一 carrier；stage2 completion 已有 contact history gate + stage3-5 transition 全部 static PASS | PASS carrier | stage3 correctness 需 smoke 确认 completion route、base stillness 与 opening progress。 |
...
| 检查项 | Origin 语义 | A2 可用 source | A2适配状态 | 开发/检查建议 |
|---|---|---|---|---|
| Grasp contact | selected hand handle contact count `>=4` | A2 `grasp` reward metric 已新增 `a2_gripper_handle_contact_sensor`，只读取 `/door_handle` 对 `arm_body7` / `arm_body8` 的 `force_matrix_w`，并用 two-sided contact reward；stage2 completion 复用该 contact source 但使用独立 boolean condition + 5-step contact history gate | PASS static implementation | transition 没有直接照搬 reward scalar；contact history gate 防止瞬时 spike 误触发。后续 smoke 再判断是否需要 aperture/pose stability。 |
| Premature/incorrect contact | Stage1 `grasp` reward 对 contact reward 取 `-abs(reward)` | A2 `_reward_grasp()` stage1 branch 已惩罚任何 handle contact magnitude；stage2+ 使用 source local `+Y` force axis 奖励两侧接触并惩罚 off-axis force；completion 要求最近 H=5 步 history 全部满足双侧 squeeze 且 signs opposite | PASS reward metric + completion input | smoke 中检查接触量级、off-axis penalty、history gate 是否合理。 |

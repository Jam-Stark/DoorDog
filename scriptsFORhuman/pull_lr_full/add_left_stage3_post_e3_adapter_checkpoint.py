#!/usr/bin/env python3
"""Add one deterministic zero-final post-E3 adapter to an H7/H9 actor."""
...
POST_E3_ADAPTER_KEYS = (
    "left_stage3_post_e3_adapter.0.weight",
    "left_stage3_post_e3_adapter.0.bias",
    "left_stage3_post_e3_adapter.2.weight",
    "left_stage3_post_e3_adapter.2.bias",
)
...
def main() -> int:
    ...
    if any(key in actor_state for key in POST_E3_ADAPTER_KEYS):
        raise ValueError("source already contains post-E3 adapter weights")

    with torch.random.fork_rng(devices=[]):
        torch.manual_seed(0)
        first = nn.Linear(391, 16)
    expanded = dict(actor_state)
    expanded[POST_E3_ADAPTER_KEYS[0]] = first.weight.detach().to(
        dtype=parent_weight.dtype
    )
    expanded[POST_E3_ADAPTER_KEYS[1]] = first.bias.detach().to(
        dtype=parent_weight.dtype
    )
    expanded[POST_E3_ADAPTER_KEYS[2]] = torch.zeros(
        9, 16, dtype=parent_weight.dtype
    )
    expanded[POST_E3_ADAPTER_KEYS[3]] = torch.zeros(9, dtype=parent_weight.dtype)
    if len(expanded) != 33:
        raise ValueError(f"H7/H9 actor must expand from 29 to 33 keys; got {len(expanded)}")

#!/usr/bin/env python3
"""Fit the H13 canonical Stage3 head from fixed-RIGHT H14 teacher capture."""
...
HEAD_PREFIX = "bilateral_stage3_canonical_head."
HEAD_KEYS = (
    "bilateral_stage3_canonical_head.0.weight",
    "bilateral_stage3_canonical_head.0.bias",
    "bilateral_stage3_canonical_head.2.weight",
    "bilateral_stage3_canonical_head.2.bias",
    "bilateral_stage3_canonical_head.4.weight",
    "bilateral_stage3_canonical_head.4.bias",
)
E4_KEY = "E4_POSITIVE_HINGE_RETAINED"
OLD_TRANSLATION_SCALE = 0.064
NEW_TRANSLATION_SCALE = 0.072
OLD_ROTATION_SCALE = 0.14
NEW_ROTATION_SCALE = 0.16
...
def _teacher_rows(trace: object, e4_env_ids: set[int]) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    for row in trace:
        ...
        row_target = teacher.get("canonical_target")
        row_pre_e3 = teacher.get("pre_e3")
        if (
            not isinstance(row_features, list)
            or len(row_features) != 58
            or not isinstance(row_target, list)
            or len(row_target) != 9
            or not isinstance(row_pre_e3, bool)
        ):
            raise ValueError("valid H14 teacher row violates 58-D feature/9-D target/pre_e3 contract")
        features.append(row_features)
        targets.append(row_target)
        pre_e3.append(row_pre_e3)
        env_ids.append(env_id)
    if not features:
        raise ValueError("no valid H14 Stage3 teacher rows from E4-positive terminal environments")
    features_tensor = torch.tensor(features, dtype=torch.float32)
    targets_tensor = torch.tensor(targets, dtype=torch.float32)
    targets_tensor[:, 3:6] *= OLD_TRANSLATION_SCALE / NEW_TRANSLATION_SCALE
    targets_tensor[:, 6:9] *= OLD_ROTATION_SCALE / NEW_ROTATION_SCALE
    if not torch.all(torch.isfinite(features_tensor)) or not torch.all(torch.isfinite(targets_tensor)):
        raise ValueError("H14 teacher features and relabeled targets must be finite")
    return (
        features_tensor,
        targets_tensor,
...
def _load_head(warm_checkpoint: Path) -> tuple[str, dict[str, torch.Tensor], nn.Sequential]:
    ...
    if not isinstance(actor_state, Mapping) or len(actor_state) != 29:
        raise ValueError("warm checkpoint must contain the exact 29-key H13 actor mapping")
    ...
    if len(parent) != 23:
        raise ValueError("H13 warm actor must retain exactly 23 frozen parent tensors")
    head = nn.Sequential(
        nn.Linear(58, 256),
        nn.SiLU(),
        nn.Linear(256, 256),
...
def _phase_weighted_mse(prediction: torch.Tensor, target: torch.Tensor, pre_e3: torch.Tensor) -> torch.Tensor:
...
def _mse_report(name: str, prediction: torch.Tensor, target: torch.Tensor, pre_e3: torch.Tensor) -> str:
...
def main() -> int:
    ...
    if support < 0.90:
        raise ValueError(f"relabeled twist target support must be >=0.90; got {support:.9g}")
    train = env_ids <= 11
    heldout = (env_ids >= 12) & (env_ids <= 15)
    if not torch.any(train) or not torch.any(heldout):
        raise ValueError("E4-positive teacher rows must populate env0-11 train and env12-15 heldout")
    ...
    for _ in range(args.epochs):
        ...
        loss = _phase_weighted_mse(head(train_features), train_targets, train_pre_e3)
        loss.backward()
        optimizer.step()
    ...
    if not parent_exact or len(fitted_state) != 29:
        raise RuntimeError("fitted actor must retain the exact 23-tensor parent and 29-key map")
    ...
    print(f"target_twist_row_within1={support:.9g}")

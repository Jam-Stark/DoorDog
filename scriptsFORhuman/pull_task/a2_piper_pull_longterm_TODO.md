...
## 11. HOMIE terminal-yaw three-rung ladder (2026-08-17 08:13 HKT — v5.5 T1 FAIL / return-to-planner)
...
- **第三 rung（HOMIE fine-tune，planner decision-required）:** rung2 failed 后 return to planner；fine-tune 未自动授权或启动，须新 planner decision。T2、formal T3、door、G2、P3/P4、dual-source eval/render 均 `NOT_RUN`，zero G3 attempts，无 passage denominator，stopping condition 未满足；diagnostics denominator=false/none。immutable `0.05 m/0.15 rad` 不得放宽。v5.2 terminal-current rule-5、v5.1 initialization-latch、v5.3 H-D 与 v5.4 scheduler facts 保持 version-scoped。唯一 formal review 仍 `FAIL`；targeted fixes/runtime acceptance 不构成 reviewer PASS 或第二轮 review。

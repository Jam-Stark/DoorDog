#!/usr/bin/env bash
# Exact64 natural evaluator for one Wave-2 full-continuation checkpoint.
set -euo pipefail

usage() {
  echo "usage: $0 0 P_S0|P_S1|P_S2 6750|7500|8250|9000 CONTINUE_TRAIN_ROOT EVAL_ROOT" >&2
}
[[ $# -eq 5 ]] || { usage; exit 2; }
...
train_root=$4
eval_root=$5
[[ "$gpu" == 0 ]] || { echo "Wave2 evaluation is bound to GPU0" >&2; exit 2; }
[[ "$cell" =~ ^P_S[012]$ && "$step" =~ ^(6750|7500|8250|9000)$ ]] || { usage; exit 2; }

seed=${cell##*_S}
checkpoint="$train_root/$cell/model_step_$(printf '%06d' "$step").pt"
[[ -f "$checkpoint" ]] || { echo "missing continuation checkpoint: $checkpoint" >&2; exit 2; }
runtime=(
...
for side in left right; do
  ...
  [[ ! -e "$output" ]] || { echo "fresh Wave2 eval output required: $output" >&2; exit 1; }
  mkdir -p "$output"
  common=(
    ...
    ++eval_name="PULL_V26_8_WAVE2_${cell}_STEP${step}_${side}" ++eval_output_dir="$output" hydra.run.dir="$output" +device=cuda:0
  )

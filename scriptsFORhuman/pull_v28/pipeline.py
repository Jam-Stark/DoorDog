#!/usr/bin/env python3
"""v28 2048-env train dispatch and one GPU0 natural-evaluation queue."""
...
RUN_GROUP = "pull_v28_rebuild_2048_20260914"
...
INITIAL_ETA_SOURCE = "temporary v28 2048 startup estimate; replace after the first formal batches"
...
def resume_config(output: Path, checkpoint: Path, attempt: int) -> Path:
    source = output / "config.yaml"
    target = output / f"resume_attempt{attempt}.yaml"
    require(source.is_file(), f"missing original materialized config: {source}")
    require(checkpoint.is_file(), f"missing explicit resume checkpoint: {checkpoint}")
    require(not target.exists(), f"resume config already exists: {target}")
    require(attempt > 0, "resume attempt must be positive")
    config = yaml.safe_load(source.read_text(encoding="utf-8"))
    require(config["checkpoint"] is None, "original materialized config must be scratch")
    require(config["checkpoint_load_mode"] == "full", "resume requires full checkpoint load")
    require(config["auto_load_latest"] is False, "resume requires explicit checkpoint selection")
    require(config["num_envs"] == config["env"]["config"]["num_envs"] == 2048, "resume requires current 2048-env contract")
    require(config["algo"]["trl"]["num_total_batches"] == 6000, "original target must remain 6000")
    require(config["callbacks"]["model_save"]["save_frequency"] == 250, "save frequency must remain 250")
    config["checkpoint"] = str(checkpoint.resolve())
    target.write_text(yaml.safe_dump(config, sort_keys=False), encoding="utf-8")
    return target


def resume(a: argparse.Namespace) -> int:
    gpu, output = CELL_GPU[a.cell], a.train_root / a.cell
    require(a.attempt > 0, "resume attempt must be positive")
    require(output.is_dir(), f"missing canonical cell directory: {output}")
    require_train_gpu(gpu)
    config = resume_config(output, a.checkpoint, a.attempt)
    verified = output / f"resume_attempt{a.attempt}_verified.yaml"
    require(not verified.exists(), f"resume compose verification already exists: {verified}")
    compose = [str(PYTHON), "-B", "-m", "gr00t.rl.train_agent_trl", "--config-dir", str(output), "--config-name", config.stem, "--cfg", "job", "--resolve"]
    with verified.open("x", encoding="utf-8") as stream:
        subprocess.run(compose, env=runtime_env(gpu), stdout=stream, check=True)
    final = output / "model_step_006000.pt"
    command = [
        str(PYTHON), str(RUNNER), "--output", str(output), "--gpu", str(gpu), "--required", final.name, "--", "env",
        *[f"{key}={value}" for key, value in runtime_env(gpu).items() if key in {"CUDA_VISIBLE_DEVICES", "CUDA_DEVICE_ORDER", "ACCELERATE_TORCH_DEVICE", "WANDB_MODE", "HYDRA_FULL_ERROR", "PYTHONUNBUFFERED", "OMP_NUM_THREADS", "PYTHONPATH"}],
        str(PYTHON), "-B", "-m", "gr00t.rl.train_agent_trl", "--config-dir", str(output), "--config-name", config.stem,
    ]
    return subprocess.run(command, cwd=REPO).returncode
...
def submit(a: argparse.Namespace, kind: str) -> int:
    require(a.attempt > 0, "attempt must be positive")
    if kind == "train":
        gpu = CELL_GPU[a.cell]
        ...
        receipt_kind = "train"
        lineage = "null scratch"
    elif kind == "resume":
        gpu = CELL_GPU[a.cell]
        command = [str(PYTHON), str(Path(__file__).resolve()), "resume", "--cell", a.cell, "--checkpoint", str(a.checkpoint), "--attempt", str(a.attempt), "--train-root", str(a.train_root), "--eval-root", str(a.eval_root)]
        suffix = a.cell.lower()
        output = a.train_root / f"{a.cell}__train_attempt{a.attempt}.supervisor.log"
        receipt_kind = "train"
        lineage = str(a.checkpoint)
    else:
        gpu = EVAL_GPU
        ...
        receipt_kind = "eval_queue"
        lineage = "null scratch"
    name = receipt_name(a.train_root, receipt_kind, suffix, a.attempt)
...
def main() -> int:
    ...
    x = sub.add_parser("resume"); x.set_defaults(func=resume); x.add_argument("--cell", required=True, choices=CELLS); x.add_argument("--checkpoint", type=Path, required=True); x.add_argument("--attempt", type=int, required=True); add_roots(x)
    ...
    x = sub.add_parser("submit-resume"); x.set_defaults(func=lambda a: submit(a, "resume")); x.add_argument("--cell", required=True, choices=CELLS); x.add_argument("--checkpoint", type=Path, required=True); x.add_argument("--attempt", type=int, required=True); add_roots(x); add_submit_timing(x)

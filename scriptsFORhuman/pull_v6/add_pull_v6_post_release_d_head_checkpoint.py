#!/usr/bin/env python3
"""Add the zero-initialized pull-v6 post-release D-only head to one actor checkpoint."""
...
D_HEAD_WEIGHT_KEY = "post_release_d_head.weight"
D_HEAD_BIAS_KEY = "post_release_d_head.bias"
...
def main() -> int:
    ...
    if D_HEAD_WEIGHT_KEY in actor_state or D_HEAD_BIAS_KEY in actor_state:
        raise ValueError(f"{source} already contains post-release D-head weights")

    d_head_state = dict(actor_state)
    d_head_state[D_HEAD_WEIGHT_KEY] = torch.zeros(
        (9, 256), dtype=rnn_weight.dtype, device=rnn_weight.device
    )
    d_head_state[D_HEAD_BIAS_KEY] = torch.zeros(9, dtype=rnn_weight.dtype, device=rnn_weight.device)

    with output.open("xb") as stream:
        torch.save({actor_key: d_head_state}, stream)
    print(f"added post-release D head to {source} -> {output} using actor key {actor_key!r}")
    return 0

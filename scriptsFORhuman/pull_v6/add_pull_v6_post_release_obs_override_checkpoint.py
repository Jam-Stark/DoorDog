#!/usr/bin/env python3
"""Add the zero-initialized pull-v6 current-observation D-only override to one actor checkpoint."""
...
OBS_OVERRIDE_WEIGHT_KEY = "post_release_obs_override.weight"
OBS_OVERRIDE_BIAS_KEY = "post_release_obs_override.bias"
...
def main() -> int:
    ...
    if OBS_OVERRIDE_WEIGHT_KEY in actor_state or OBS_OVERRIDE_BIAS_KEY in actor_state:
        raise ValueError(f"{source} already contains post-release observation-override weights")

    override_state = dict(actor_state)
    override_state[OBS_OVERRIDE_WEIGHT_KEY] = torch.zeros(
        (9, 135), dtype=rnn_weight.dtype, device=rnn_weight.device
    )
    override_state[OBS_OVERRIDE_BIAS_KEY] = torch.zeros(9, dtype=rnn_weight.dtype, device=rnn_weight.device)

    with output.open("xb") as stream:
        torch.save({actor_key: override_state}, stream)
    print(f"added post-release observation override to {source} -> {output} using actor key {actor_key!r}")
    return 0

#!/usr/bin/env python3
"""Add the zero-initialized pull-v6 release residual to one strict 135-D actor checkpoint."""
...
def main() -> int:
    ...
    if RESIDUAL_KEY in actor_state:
        raise ValueError(f"{source} already contains {RESIDUAL_KEY}")

    rnn_weight = _require_tensor(actor_state, RNN_INPUT_KEY, (1024, 135))
    residual_state = dict(actor_state)
    residual_state[RESIDUAL_KEY] = torch.zeros_like(rnn_weight[0, -2:])

    with output.open("xb") as stream:
        torch.save({actor_key: residual_state}, stream)
    print(f"added {RESIDUAL_KEY} to {source} -> {output} using actor key {actor_key!r}")
    return 0

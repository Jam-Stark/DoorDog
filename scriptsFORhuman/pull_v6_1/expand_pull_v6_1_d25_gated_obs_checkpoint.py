#!/usr/bin/env python3
"""Insert zero-initialized hinge-velocity and D25-control columns before release mode."""
...
def _insert_vector(value: torch.Tensor, fill: float) -> torch.Tensor:
    return torch.cat((value[:133], value.new_full((2,), fill), value[133:]))


def _insert_columns(value: torch.Tensor) -> torch.Tensor:
    return torch.cat((value[:, :133], value.new_zeros((value.shape[0], 2)), value[:, 133:]), dim=1)
...
def main() -> int:
    ...
    expanded = dict(state)
    expanded[MEAN_KEY] = _insert_vector(_require(state, MEAN_KEY, (135,)), 0.0)
    expanded[VAR_KEY] = _insert_vector(_require(state, VAR_KEY, (135,)), 1.0)
    expanded[RNN_KEY] = _insert_columns(_require(state, RNN_KEY, (1024, 135)))
    expanded[HEAD_KEY] = _insert_columns(_require(state, HEAD_KEY, (9, 135)))
    with output.open("xb") as stream:
        torch.save({actor_key: expanded}, stream)
    print(f"expanded {source} -> {output} with hinge/D25 columns 133:135")
    return 0

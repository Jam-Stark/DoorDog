...
if [[ $# -ne 3 ]]; then
    echo "usage: $0 GPU LABEL CHECKPOINT" >&2
    exit 2
fi
...
label=$2
checkpoint=$3
video_root="$repo_root/logs_eval/base_v25/m3/teacher_videos/$label"

for side in left right; do
    output_dir="$video_root/$side"
    env -u CUDA_VISIBLE_DEVICES \
        ...
        ++eval_name="V25_M3_VIDEO_${label}_${side}" \

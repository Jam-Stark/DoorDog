...
for factor in O0 O1; do
  for side in left right; do
    env CUDA_VISIBLE_DEVICES=2 CUDA_DEVICE_ORDER=PCI_BUS_ID PYTHONPATH="$repo" "$python_bin" -B "$repo/scriptsFORhuman/v26_5/v26_5_wave1_runtime_contract_case.py" --device cuda:0 --headless --factor "$factor" --side "$side" --output "$cases/${factor}_${side}.json"
  done
done

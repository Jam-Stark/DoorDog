...
usage() { echo "usage: $0 GPU LABEL OUTPUT_ROOT SEED left|right"; }
...
gpu=$1; label=$2; output_root=$3; seed=$4; side=$5
[[ "$gpu" =~ ^[0-9]+$ && "$label" =~ ^K0_CONT_STEP2000_O0A0_S[01]$ && "$seed" =~ ^[01]$ && "$side" =~ ^(left|right)$ && -f "$source_checkpoint" ]] || { usage >&2; exit 2; }
[[ "${label##*_S}" == "$seed" ]] || { echo "label/seed mismatch" >&2; exit 2; }
output="$output_root/$label/$side"
[[ ! -e "$output" ]] || { echo "refusing to overwrite Wave2 K0 eval: $output" >&2; exit 1; }
env CUDA_VISIBLE_DEVICES="$gpu" CUDA_DEVICE_ORDER=PCI_BUS_ID ACCELERATE_TORCH_DEVICE=cuda:0 WANDB_MODE=disabled HYDRA_FULL_ERROR=1 PYTHONUNBUFFERED=1 PYTHONPATH="$repo" \
  "$python_bin" -B -m gr00t.rl.eval_agent_trl +ablation=wbmanip/base_v26_5_wave2_K0_eval_O0A0 \
  ...
  ++eval_name="V26_5_WAVE2_${label}_${side}" ++eval_output_dir="$output"
for required in metrics_eval.json a2_v14_per_env_records.json stage2_5_step_trace.json a2_eval_diagnostic_metadata.json; do
  [[ -f "$output/$required" ]] || { echo "missing required K0 eval artifact: $output/$required" >&2; exit 1; }
done

...
[[ $(grep -Fc "trace_reward_terms='[push_door_handle,a2_stage3_unlatch_hold,push_door_hinge,a2_stage3_stage4_hold_and_drive]'" "$side_script") -eq 2 ]]
! grep -Fq 'a2_stage3_handle_creation' "$side_script"
grep -Fq '++algo.config.eval.a2_diagnostic_reward_terms="$trace_reward_terms"' "$side_script"
[[ $(grep -Fc 'handle_depression_scale=0.0' "$side_script") -eq 2 ]]
[[ $(grep -Fc 'handle_creation_scale=0.0' "$side_script") -eq 2 ]]

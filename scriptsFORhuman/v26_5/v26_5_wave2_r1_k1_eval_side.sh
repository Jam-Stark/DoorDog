...
case "$view" in
  control)
    selector=wbmanip/base_v26_5_eval_O0A0
    identity_control=true
    residual_policy_only=false
...
env CUDA_VISIBLE_DEVICES="$gpu" CUDA_DEVICE_ORDER=PCI_BUS_ID ACCELERATE_TORCH_DEVICE=cuda:0 WANDB_MODE=disabled HYDRA_FULL_ERROR=1 PYTHONUNBUFFERED=1 PYTHONPATH="$repo" \
  ...
  ++algo.config.eval.a2_v23_p06_policy_only=false ++algo.config.eval.a2_v26_5_policy_only_identity_control="$identity_control" ++algo.config.eval.a2_v26_5_policy_only_residual="$residual_policy_only" ++algo.config.eval.a2_v26_5_runtime_load_receipt=true \
  ...
  ++algo.config.eval.a2_diagnostic_reward_terms="$trace_reward_terms" \
  ++env.config.a2_v26_2_handle_depression_scale="$handle_depression_scale" ++env.config.a2_v26_3_handle_creation_scale="$handle_creation_scale" \
...
for artifact in metrics_eval.json a2_v14_per_env_records.json stage2_5_step_trace.json a2_eval_diagnostic_metadata.json a2_v26_5_runtime_load_receipt.json; do [[ -f "$output/$artifact" ]] || { echo "missing K1 artifact: $output/$artifact" >&2; exit 1; }; done

...
case ${1:-} in
  ...
  wiring-gate) [[ $# -eq 2 && $2 == --launch ]] || { usage >&2; exit 2; }; static_verify; gpu_idle 4; output="$stage/wiring_gate/R12_DUAL_S0_RIGHT"; [[ ! -e "$output" ]] || { echo "wiring output exists" >&2; exit 1; }; printf -v command '%q ' bash "$repo/scriptsFORhuman/v26_5/v26_5_wave2_r1_r12_wiring_gate.sh" 4 "$output"; launch "${run_id}_wiring_gate" 4 "$command" "$runtime_logs/wiring_gate.log" "$output/metrics_eval.json" ;;
  k1-cell) [[ $# -eq 3 && $3 == --launch ]] || { usage >&2; exit 2; }; static_verify; require_pass "${run_id}_wiring_gate"; label=$2; case "$label" in K1_S0) gpu=4;seed=0;stagger=0;;K1_S1) gpu=5;seed=1;stagger=600;;*) usage >&2;exit 2;;esac; gpu_idle "$gpu"; output="$eval_root/K1/control/$label/right/metrics_eval.json"; [[ ! -e "$eval_root/K1/control/$label" && ! -e "$eval_root/K1/dual/$label" ]] || { echo "K1 output exists: $label" >&2; exit 1; }; printf -v command '%q ' bash "$repo/scriptsFORhuman/v26_5/v26_5_wave2_r1_k1_cell.sh" "$gpu" "$label" "$eval_root" "$seed"; [[ $stagger -eq 0 ]] || command="sleep $stagger; $command"; launch "${run_id}_k1_s${seed}" "$gpu" "$command" "$runtime_logs/k1/$label.log" "$output" ;;

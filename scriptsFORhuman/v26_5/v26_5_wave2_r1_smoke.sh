...
gpu=$1; output=$2
[[ "$gpu" =~ ^(2|3)$ && -f "$source_checkpoint" && ! -e "$output" ]] || { echo "R1 smoke requires source and a new output" >&2; exit 1; }
mkdir -p "$output"
common=(+exp=wbmanip/door_open_a2_base_lstm +ablation=wbmanip/base_v26_5_wave2_R1_policy_residual checkpoint="$source_checkpoint" checkpoint_load_mode=policy_only policy_only_load_actor_rms=true auto_load_latest=false seed=0 num_envs=64 algo.trl.num_total_batches=1 callbacks.model_save.save_frequency=1 env.config.a2_v26_side_permutation_seed=0 headless=true use_wandb=false simulator.config.render_results=false simulator.config.cameras.enable_cameras=false experiment_dir="$output" output_dir="$output/output" project_name=base_v26_5_wave2_r1 experiment_name=V26_5_R1_SMOKE64_B1 v26_cell=V26_5_R1_SMOKE64_B1 v26_phase=V26_5_WAVE2_R1_SMOKE)
...
[[ -f "$output/model_step_000001.pt" ]] || { echo "R1 smoke lacks one-batch checkpoint" >&2; exit 1; }

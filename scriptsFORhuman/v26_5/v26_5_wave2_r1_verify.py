...
def main() -> None:
    ...
    shadow=json.loads((a.registry.parent/"r12_cpu_shadow_gate.json").read_text(encoding="utf-8"))
    require(
        shadow.get("schema") == "a2_piper_base_v26_5_r12_cpu_shadow_gate_v2"
        and shadow.get("status") == "PASS"
        and shadow.get("checkpoint_path") == str(SOURCE)
        and shadow.get("source_config_path") == str(SOURCE.parent / "config.yaml")
        and shadow.get("r12_selector_path") == str(ROOT / "gr00t/rl/config/ablation/wbmanip/base_v26_5_wave2_R1_policy_residual.yaml")
        and shadow.get("actor_state_key") == "policy_state_dict"
        and shadow.get("state_transfer") == "legacy_exact_without_residual"
        and shadow.get("comparison_hidden_state") == "both_actors_reset_to_none_before_each_comparison"
        and shadow.get("rms_source_fields") == ["running_mean_std.running_mean", "running_mean_std.running_var", "running_mean_std.count"]
        and shadow.get("residual_pose_slice") == [83, 101]
        and shadow.get("resolved_main_geometry_target_enabled") is False
        and shadow.get("zero_identity_within_tolerance") is True
        and all(shadow.get(key) is True for key in ("missing_residual_key_rejected","stage_mismatch_rejected","frozen_base_grad_free","residual_grad_present"))
        and all(isinstance(shadow.get(key),(int,float)) and shadow[key] <= 1e-6 for key in ("two_d_mean_max_abs","sequence_mean_max_abs","rollout_mean_max_abs","rollout_std_max_abs")),
        "R12 measured real-checkpoint CPU shadow gate mismatch",
    )
    ...
    require(k1.get("pre_k1_wiring_gate")=={"view":"dual","seed":0,"side":"right","physical_gpu":4,"num_envs":64,"max_episode_length_s":0.02,"control_ticks":2,"required_artifacts":["a2_v26_5_runtime_load_receipt.json","a2_eval_diagnostic_metadata.json","a2_v14_per_env_records.json","metrics_eval.json"]}, "R12 two-control-tick wiring contract mismatch")
    ...
    partial_expected={"v26_schema":"a2_piper_base_v26_5_wave2_r1_eval_policy_residual_v1","env.config.a2_v26_5_geometry_target_enabled":False,"env.config.a2_v26_5_actor_gauge_enabled":True,"algo.config.actor._target_":"gr00t.rl.trl.modules.actor_critic_modules_recurrent.A2V26_5PolicyResidualRecurrentActor","algo.config.actor.input_key":"actor_obs","algo.config.actor.residual_input_key":"residual_actor_obs"}
    ...
    require(partial.get("obs.obs_dict.actor_obs")==RAW_OBS and partial.get("obs.obs_dict.residual_actor_obs")==GAUGE_OBS, "R12 eval partial dual observation provenance mismatch")
    for name,path in paths.items():
        ...
        if name.endswith("train"):
            expected.update({"env.config.a2_v26_4_side_canonicalization_enabled":False,"env.config.a2_v26_5_geometry_target_enabled":False,"env.config.a2_v26_5_stage3_delta_rebase_enabled":False,"env.config.a2_v26_5_actor_gauge_enabled":True,"algo.config.actor._target_":"gr00t.rl.trl.modules.actor_critic_modules_recurrent.A2V26_5PolicyResidualRecurrentActor","algo.config.actor.input_key":"actor_obs","algo.config.actor.residual_input_key":"residual_actor_obs","algo.config.actor.residual_stage_obs_slice":[127,133],"algo.config.actor.residual_hidden_dim":128})
        for key,value in expected.items(): require(table.get(key)==value, f"{name}: {key}={table.get(key)!r}, expected {value!r}")
        if name.endswith("train"): require(table.get("obs.obs_dict.actor_obs")==RAW_OBS and table.get("obs.obs_dict.residual_actor_obs")==GAUGE_OBS, f"{name}: R12 dual observation provenance mismatch")

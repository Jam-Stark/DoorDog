#!/usr/bin/env bash
# New v26-7 G2 evaluator: deliberately separate from v26-6 because this phase
# reserves GPU0--1 while the historical launcher accepts only GPU4--7.
set -euo pipefail
[[ $# -eq 5 ]] || { echo "usage: $0 GPU B0_S0|B0_S1|B1_S0|B1_S1 250|500|750 TRAIN_ROOT OUTPUT_ROOT" >&2; exit 2; }
repo=/home/baoquanc/workspace/DoorDog-A2_Piper
py=/home/baoquanc/anaconda3/envs/isaaclab/bin/python
gpu=$1; cell=$2; step=$3; train_root=$4; output_root=$5
[[ "$gpu" =~ ^[01]$ ]] || { echo "G2 is bound to GPU0 or GPU1" >&2; exit 2; }
[[ "$cell" =~ ^B[01]_S[01]$ && "$step" =~ ^(250|500|750)$ ]] || exit 2
seed=${cell##*_S}
checkpoint="$train_root/$cell/model_step_000$step.pt"
[[ -f "$checkpoint" ]] || { echo "missing Wave-B checkpoint: $checkpoint" >&2; exit 2; }
label="${cell}_STEP0${step}"
for side in left right; do
  output="$output_root/$label/$side"
  [[ ! -e "$output" ]] || { echo "refusing to overwrite G2 output: $output" >&2; exit 1; }
  env CUDA_VISIBLE_DEVICES="$gpu" CUDA_DEVICE_ORDER=PCI_BUS_ID ACCELERATE_TORCH_DEVICE=cuda:0 \
    ...
    "$py" -B -m gr00t.rl.eval_agent_trl \
      ...
      ++eval_name="V26_7_G2_${label}_${side}" ++eval_output_dir="$output"
  for required in metrics_eval.json a2_v14_per_env_records.json stage2_5_step_trace.json a2_eval_diagnostic_metadata.json; do
    [[ -f "$output/$required" ]] || { echo "missing G2 artifact: $output/$required" >&2; exit 1; }
  done
done

...
baseline_static_lock="$repo/scriptsFORhuman/v26_8/runtime_logs/v26_8_bilateral_opening_scaffold_decay_20260903/source_lock.json"
r2_contract_lock="$runtime_logs/r2_contract_lock.json"
g0_root="$repo/logs_eval/base_v26/v26_8_bilateral_opening_scaffold_decay_20260903/G0_static_unit"
g1_root="$eval_root/G1_k_wiring"
p0_assets="$repo/scriptsFORhuman/v26_8/v26_8_p0_assets.py"
r2_verify="$repo/scriptsFORhuman/v26_8/v26_8_r2_verify.py"
cells=(C_S1 W_S1 K_S1 C_S2 W_S2 K_S2)
usage() { echo "usage: $0 static|g0|g1-launch|g1-finalize|train-launch|train-finalize|milestone-launch STEP|milestone-finalize STEP|closure|wave2-branches" >&2; }
run_name() { echo "${1}_r2"; }
...
case ${1:-} in
static)
  [[ $# -eq 1 && -f "$baseline_static_lock" && ! -e "$static_lock" && ! -e "$r2_contract_lock" ]] || { usage; exit 2; }; mkdir -p "$runtime_logs"
  "$isaac_py" "$r2_verify" --baseline "$baseline_static_lock" --output "$r2_contract_lock"
...
g1-launch)
  [[ $# -eq 1 && -f "$static_lock" && -f "$r2_contract_lock" && -f "$g0_root/g0_unit.json" && ! -e "$g1_root" ]] || { usage; exit 2; }
  require_gate "$static_lock" STATIC_PASS; require_gate "$r2_contract_lock" R2_CONTRACT_PASS; require_gate "$g0_root/g0_unit.json" G0_PASS
  gpu_visible 0; mkdir -p "$runtime_logs/g1"
  printf -v command '%q ' bash "$repo/scriptsFORhuman/v26_8/v26_8_g1_wiring_gate.sh" 0 "$g1_root"
  launch v26_8_g1_wiring 0 "$command" "$runtime_logs/g1/g1.log" "$g1_root/g1_wiring.json" ;;
g1-finalize)
  [[ $# -eq 1 ]] || { usage; exit 2; }; "$py" "$supervisor" finalize --receipt "$(receipt v26_8_g1_wiring)"; require_receipt_pass v26_8_g1_wiring; require_runtime_returncodes_pass "$g1_root/K_S1_smoke/v26_8_runtime_process_receipt.json"; require_gate "$g1_root/g1_wiring.json" G1_PASS ;;
train-launch)
  [[ $# -eq 1 && ! -e "$train_root" ]] || { usage; exit 2; }; require_gate "$static_lock" STATIC_PASS; require_gate "$r2_contract_lock" R2_CONTRACT_PASS; require_gate "$g0_root/g0_unit.json" G0_PASS; require_gate "$g1_root/g1_wiring.json" G1_PASS; mkdir -p "$train_root" "$runtime_logs/train"
...
milestone-launch)
  [[ $# -eq 2 ]] && valid_step "$2" || { usage; exit 2; }; step=$2; root="$eval_root/milestones/step$step"; [[ ! -e "$root" ]] || { echo "fresh milestone root required: $root" >&2; exit 1; }; require_gate "$g1_root/g1_wiring.json" G1_PASS
...
unexpected=[]; states={}
for cell in cells:
    receipt=run_root/f"v26_8_train_{cell.lower()}_r2"/"RUN_RECEIPT.json"
...
for cell in sys.argv[3:]:
    receipt=json.load(open(root/f"v26_8_train_{cell.lower()}_r2"/"RUN_RECEIPT.json",encoding='utf-8'))
    if cell in stopped:

...
当前里程碑（2026-09-21 20:04 HKT）：[D076](/home/baoquanc/workspace/DoorDog-A2_Piper/.ai/runtime/v29_baseline_team/D076_C002_RESUME7000_NATURAL_MILESTONE.json)已核对7000自然64：LEFT31/32goal、RIGHT31/32到Stage3但尚无Stage4；原D074继续至8000。下方6000的0/64和阶段卡点保留为历史，B08修复仍与本次运行输入隔离。

更新：2026-09-21 HKT。D072已关闭[C002冻结实现与首轮训练/评估交付](a2_piper_base_v29_C002_final_readout_20260921.md)：6000 batch完成，最终64自然首episode为0/64 goal；执行接受不等于策略成功。**B08已完成公共修复并同步四个开发worktree**，见[B08实施记录](a2_piper_v29_B08_implementation_20260921.md)；现有GPU0/GPU1实验继续使用冻结输入。下文B01–B07早期“待实施/未批准”保留历史语境，当前实现/执行状态以D056/D060/D072为准。

历史更新：2026-09-19 01:25 HKT，D030–D031确定完整baseline worker实施/监督与planner验收闭环，当时待worker任务ID，正式GPU0训练尚未批准。

状态：原C002_6000_DELIVERY_CLOSED；D074_RESUME8000_DELIVERY_CLOSED；OBSERVED_NATURAL64_OF64；B08_IMPLEMENTED_LOCAL。Owner已直接授权同配方6000→8000及7000/8000各64自然首episode，见[原指令/启动](../../.ai/runtime/v29_resume8000/OWNER_DECISION_AND_START.md)与[精确计划](../../.ai/runtime/v29_resume8000/OWNER_AUTHORIZED_PLAN.json)。D072原结果保留，D077已记录8000自然64/64的有限样本结果；本轮不越过8000，D074的运行输入保持原B08行为及reward/action/assets；B08开发修复已独立隔离。

2026-09-18 Pro回传已完成本地解析，原包/原文、来源、参数及复核附件见[本次归档](../pro_reviews/v29/20260918_102906__baseline_and_v28_pro_review/README.md)，详细核对见[LOCAL_RECONCILIATION](../pro_reviews/v29/20260918_102906__baseline_and_v28_pro_review/LOCAL_RECONCILIATION.md)。其中“六项未决”是回传解析时的状态；Owner后续确认以本清单与D008–D031为准，不改Pro原文。
...
- [x] **~~B08：Stage0 arm动作被逐步清零覆盖~~ — 实现完成，公共分支同步完成；CPU定向证明通过。**
  - **Owner要求（2026-09-21，D075）**：完成修复，同时更新N01、N02、B05-ablation并保持公共逻辑一致；9月20日的只读确认已转为本轮实施。
  - **改动**：普通与canonical两条动作路径均删除Stage0在线覆盖，policy六维arm增量能持续积分。保留真实reset初始化、原动作历史/限幅/映射；Stage0默认姿态reward和晋级条件未改。无新开关、fallback或N01特例。
  - **证据与目录**：[完整实施记录](a2_piper_v29_B08_implementation_20260921.md)。CPU实际生产step提取路径在Stage0连续三步累计0.18，RIGHT镜像及选定环境reset正常；四个开发worktree公共逻辑一致。未新增IsaacSim、训练/评估或策略收益证据。
  - **运行隔离**：GPU0 C002和GPU1 HA-C001的原运行目录保持冻结源码；baseline修复及B05-ablation修复使用独立开发worktree，N01/N02已直接同步。本地改动未commit/push；既有候选tag和配方保留。
  - **原问题事实（INSPECTED）**：C002普通路径在每步积分后将Stage0六维arm累计量清零；canonical路径恢复physical origin，随后按`q_target = default_dof_pos + action_scale * action`生成目标。这是在线命令目标覆盖，不是物理位置瞬移，也不等于policy学会保持姿态。其对已观察训练困难的贡献尚未量化。
...
- [x] **~~B05：handle 几何族与概率末端回钩~~ — PASS（七族全部确认，方案已入plan）。** 保持双指夹持lever的基本方式，采用F0–F6七族与独立回钩选项。
  - **最新Owner决定（D029）**：全部按Pro建议采用，已完成当前PiPER/TCP第二次几何确认并落实[执行规格](a2_piper_base_v29_b05_handle_design.md)与[worker交接](a2_piper_base_v29_b05_worker_handoff.md)。七族1/7、h55–85、全截面/roll/λ、条件圆滑return、同轴rose及X1训练外配置均已确定；下面D025–D028记录保留当时过程。
  - **二次确认**：入口约70mm/TCP约74mm净开口支持最大34.071033mm截面；主杆/掌基及默认G下rose/return分离已核。回钩按有效联合域直接采样；证据和适用条件见[确认记录](b05_gripper_confirmation_20260919/README.md)。本轮未生成仿真资产或改训练代码。
  ...
  - **Owner决定（D026）**：七族全部apply到baseline plan，取消首批/扩展拆分；本项方案确认完成。Pro已于D028回传并完成定向核对，参数细化不撤销本项勾选。
  - **职责与证据**：Main默认只负责planner工作；本轮不做代码实施或训练监督。没有导入后碰撞、左右扫掠或抓取表现证据，不将方案PASS写成运行/泛化PASS。
  - **Pro回包（D028）**：[原文/本地整合](../pro_reviews/v29/20260918_233820__B05_handle_review/README.md)。建议保留七族、暂不加第八训练族；F5局部接触重叠最大，优先h/截面方向/条件return，X1轻弯×椭圆留未见组合候选。
  - **核对与待决**：约0.98/1.10mm曲率残差、F6约2.42mm厚度梯度及50.8mm指体前伸获静态支持；h55仅有4.2mm理想平面余量。上述参数在D028时未采纳，现已由D029全量确认，详见plan §7；厂家projection不直接当h。
  ...
  - **后续细化/实现事实**：七族及统一生成器目标frame方案已确认；连续尺寸域、族/回钩配比已按D029确定。真实指部插入、产品图纸截面/曲率和动态净空仍需对应证据，与方案结案分开。
...
- [x] **Owner同配方续训与自然趋势评估（D074）— D077接受并关闭**：6000→8000及7000/8000自然64均完成；总goal0→31→64，8000左右各32/32。命令/预算、exact8000 end capture及full loader、自然/staged0口径已核对；GPU0自有lease与逻辑等待关闭。单seed有限人口，不自动延长或追加seed；[最终读回](a2_piper_base_v29_C002_resume8000_final_readout_20260922.md)。

统一工作入口：[完整worker启动prompt](a2_piper_base_v29_worker_start_prompt.md)；实施后的逐项验收由[planner合同](a2_piper_base_v29_acceptance_and_coordination.md)管理，B05文档仅作技术附件。

- [x] **B01物理实现**：D056/D060已验收三档质量、closer各半与native摩擦联合配方；D072保留该实现证据范围。
- [x] **B04限位实现**：D056/D060已验收每门U(90°,150°)的native upper、metadata与有界恢复证据；旧“仍150°”描述为实现前历史。

- [x] **B05完整实现**：D056/D060已验收七族、批准参数、联合return/rose、G/FixedJoint/consumer及记录内真实接触操作路径；不扩大为任意动态抓握保证。
- [x] **冻结C002 baseline planner验收**：D056/D060的逐项实现验收范围由D072保留，本轮交付关闭；后来发现的B08已由D075独立修复，不将本项勾选解释为不存在已知问题。
- [x] **GPU0首轮训练与监督**：D060批准的一次seed291/4096/6000及64自然首episode评估实际完成，D072接受执行交付；0/64 goal，尚未展示整任务策略成功。没有追加训练授权。
...
B08共同动作接口修复已由D075完成；后续训练效果另按授权评估。它与下列既有议题分别跟踪。

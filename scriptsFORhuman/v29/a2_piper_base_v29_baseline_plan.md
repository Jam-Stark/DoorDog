...
更新：2026-09-18 21:38 HKT。D026确认B05七族全部进入baseline plan并标方案完成；B04同按规划结案。默认planner职责与独立Pro审阅范围已记录。

状态：**逐项落地中，尚未整体冻结**。当前baseline恢复D021之前的门轴、hold_and_drive和grasp公式，不实施按需重抓/回弹恢复奖励。B04方案讨论已完成、最大角代码待实施；B01物理代码待实施，B05七族全部确认、方案讨论完成，独立Pro审阅待返回。B02后置独立ablation，B03保持15°并后置；N02承接重新抓把手扶门议题。训练预算与资格规则尚未批准。
...
## 7. B05：七族全部纳入baseline的把手与grasp target方案

**Owner已确认（D026）**：F0圆直杆、F1椭圆直杆、F2圆角扁直杆、F3单弧杆、F4浅S杆、F5偏置直腹杆、F6缓变锥度杆全部进入本次baseline plan。B05标记PASS（方案讨论完成）；不再采用F0–F3首批、F4–F6后续的拆分。完整标称尺寸和逐族I/J/G见[设计规格](a2_piper_base_v29_b05_handle_design.md)与[七族概览](b05_designs_20260918/handle_family_atlas.png)。

自由主握段I与TCP中心候选J分开：标称I长65–74mm，以当前原始指体切向包络约56mm加每端3mm工程余量收缩，J长3–12mm，G取J弧长中点。这是静态设计参照，实际曲杆仍需结合指体几何判断。G的位置和切线同源：F3抬高5.68mm，F4局部切线约−8.25°，F5抬高8mm。

统一目标frame采用p=γ(s_g)，t指向轴颈，a为接近方向，c=t×a，R=[−t,c,a]映射PiPER Y闭合/+Z接近。后续由生成器一次输出frame并同步FixedJoint，consumer去除对应重复固定旋转/LEFT补偿，pregrasp沿目标局部−Z退100mm。保留同一handle刚体及双指contact语义；该接入尚未实施。

回钩独立于主族，不作为第八族或新抓法；当前源码50%概率、40–60mm回返是审阅对照，不误报新增能力。visual/collision来自同一几何，质量/COM/惯量与组合形状一致。七族标称设计已确认；族权重、连续尺寸域、截面/回钩细节由本次Pro提出简洁建议，不能把未定概率写成Owner批准的现实分布。

**独立Pro审阅**：检查全局轮廓与抓点邻域是否实质重复；判断是否需要最少量补充构型；搜集厂家产品/图纸，比较长度、厚度、截面、曲率/偏置、门板站距、回返与自由抓握区，判断如何保持sim模型简单并扩大有效覆盖。对合并/新增给具体理由；不能把形态覆盖提升写成已证明的策略泛化收益。该审阅是已确认方案的后续优化，不使B05重新变成未完成。

本任务默认由Main担任planner；未获Owner新的明确要求，不自动进行代码实施、训练启动或训练监督。本轮仅维护方案与Pro交付。

## 8. 当前未结与后续分支

本次baseline：B01已确认范围、物理代码待实施；B04方案已讨论完成、最大角代码待实施，奖励已恢复原公式。B05七族与目标设计已全部确认、方案讨论PASS，Pro独立审阅待返回；已有B06/B07及Stage0速度决定沿用；B03保持15°并后置。强回弹后重抓把手已移到N02讨论。
